...
def GRITI_import_AMPERE(dates,folder,dateRange_dayNum_full_adj,FLG_dataMix=0,FLG_dataPreference=0,FLG_float64=0):
        ...
        if( AMPERE_sourceIndex == 0 ): #this means it is the first type that has been refactored into an HDF5 file
            ...
            if( cntr == AMPERE_raws_len ):
                ...
                print('File identified as the date '+str(dateRange_dayNum_full[i,0])+str(dateRange_full[i,1]).zfill(2)+str(dateRange_full[i,2]).zfill(2)+
                      ...
                      folder[1] + '\\' + folder_AMPERE + '\\' + str(dateRange_full[i,0]) + '\\' + AMPERE_fileName+'\nGonna crash now.');
                #return("No"); #return something that will def crash things
                import sys #yolo import
                sys.crash(); #more def will crash
            #END IF
            ...
            dataList_deltas = np.append(np.insert(np.where(np.diff(dataList)>1)[0],0,0),AMPERE_raws_len); #get the line skips where there are the headers occur
            ...
            for j in range(0,dataList.size):
                ...
                #END IF
                if( indxr > dataList_deltas[cntr_long+1] ): #check to increment long cntr
