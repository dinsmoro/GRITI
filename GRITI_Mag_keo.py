...
from GRITI_Mag_keo_area import GRITI_Mag_keo_area
# from GRITI_Mag_keo_fancyPlot_area import GRITI_Mag_keo_fancyPlot_area
from GRITI_keo_subfun_raytrace import GRITI_keo_subfun_raytrace


def GRITI_Mag_keo(data,dates,settings,FLG_disablePlot=0):
    
    #==============UNPACK SETTINGS==============
    keo_angle = np.copy(settings['Mag']['keo angle']).item(); #make a copy just in case python memory stuff
    keo_width = np.copy(settings['Mag']['keo width orig']).item(); #make a copy just in case python memory stuff
    ...
    cntr = 0; #prep cntr
    for j in range(0,len(siteNames)):
        cntr += data['Mag'][siteNames[j]]['magF'].size; #get the total size
    #END FOR j
    Mag_time = np.zeros( cntr, dtype=np.float64); #preallocate
    Mag_deltaMagF = np.zeros( cntr, dtype=np.float64); #preallocate
    Mag_lat = np.zeros( cntr, dtype=np.float64); #preallocate
    Mag_long = np.zeros( cntr, dtype=np.float64); #preallocate
    cntr = 0; #Prep cntr
    maxRec = 0; #prep max recorder
    for j in range(0,len(siteNames)):
        tempSize = data['Mag'][siteNames[j]]['magF'].size; #prep
        Mag_time[cntr:cntr+tempSize] = np.float64(data['Mag'][siteNames[j]]['dayNumF']) + np.float64(data['Mag'][siteNames[j]]['secF'])/86400; #days, get unique times (v useful)
        ...
        Mag_deltaMagF[cntr:cntr+tempSize] = temp_deltaMagF; #write data
        Mag_lat[cntr:cntr+tempSize] = np.tile(data['Mag'][siteNames[j]]['lat'], tempSize); #tile data (lat doesn't change)
        Mag_long[cntr:cntr+tempSize] = np.tile(data['Mag'][siteNames[j]]['long'], tempSize); #tile data (long doesn't change)
        cntr += tempSize; #increment
        if( np.nanmax(np.abs(temp_deltaMagF)) > maxRec ):
            maxRec = np.nanmax(np.abs(temp_deltaMagF)); #set it
        #END IF
    #END FOR j
    if( settings['Mag']['keo normalize'] == 1 ):
        ...
        for j in range(0,len(siteNames)):
            tempSize = data['Mag'][siteNames[j]]['magF'].size; #prep
            Mag_deltaMagF[cntr:cntr+tempSize] = Mag_deltaMagF[cntr:cntr+tempSize]*(maxRec/np.nanmax(np.abs(Mag_deltaMagF[cntr:cntr+tempSize]))); #normalize
    ...
    print('Analyis: Magnetometer Keogram Beginning:'); 
    tic = time.time();
    ...
    if( FLG_disablePlot == 0 ):
        ...
        GRITI_Mag_keo_area(data, dates, settings, keo_width, keo_range, temp_lats_up, temp_longs_up, temp_lat_list, temp_long_list); #plot the area
    #END IF
    if( FLG_fancyPlot == 1 ):
        #THIS IS TO VIEW DATA BEING AVERAGED ~fancily~
        pass;
        # GRITI_Mag_keo_fancyPlot_area(data, dates, settings, keo_range, temp_lats_up, temp_longs_up, temp_Lat_List, temp_Long_List); #plot the area
    #END IF
    ...
    P1PtVxy = np.array([Mag_long - keo_range[0,1] , Mag_lat - keo_range[0,0]]); #effort to do own in polygon
    ...
    P3PtVxy = np.array([Mag_long - keo_range[3,1] , Mag_lat - keo_range[3,0]]); #effort to do own in polygon
...
#    #END FOR j
    ...
    Mag_time =  Mag_time[keepr];
    Mag_deltaMagF =  Mag_deltaMagF[keepr];
    Mag_lat = Mag_lat[keepr];
    Mag_long = Mag_long[keepr];
    Mag_timeUnique = np.unique(Mag_time); #days, get the unique times for the conglomeration
    ...
    Mag_keo = GRITI_keo_subfun_raytrace(Mag_time,Mag_timeUnique,Mag_lat,Mag_long,temp_lat_list,temp_long_list,Mag_deltaMagF,keo_N,sqrtApproxAo,sqrtApproxBo,P1P2Vxy,P1P4Vxy,P3P2Vxy,P3P4Vxy,P1P2Vm,P1P4Vm,P3P2Vm,P3P4Vm,P1P2P1P4dotCosT,P3P2P3P4dotCosT); #call a function to avg all the TEC together into bands
...
#    #END FOR i
    ...
    return Mag_keo, Mag_timeUnique, (Mag_timeUnique-dates['date range zero hr dayNum'][1])*24, keo_angle, keo_width, keo_range_chunks_long_plot, keo_range_chunks_long_plot_name