...
from GRITI_TEC_avgPtACCELERATOR import GRITI_TEC_avgPtACCELERATOR
...
#    avgPt_vTEC_interped[jk] = np.interp(jk,jl,avgPt_vTEC[jl]); #fill in the gaps with the spline fill in
    ...
    try:
        avgPt_vTEC_HP_noInterp = subfun_highpass( (avgPt_vTEC_time_noInterp-dateRange_dayNum_zeroHr[1])*24 , avgPt_vTEC_noInterp); #highpass that data to boot [this is inherently wrong b/c of missing data points - if low % should not be too wrong]
    except:
    ...
    avgPt_vTEC_HP = subfun_highpass( (avgPt_vTEC_time-dateRange_dayNum_zeroHr[1])*24 , avgPt_vTEC); #highpass that data to boot
