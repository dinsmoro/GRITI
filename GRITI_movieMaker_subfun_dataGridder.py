...
def GRITI_movieMaker_subfun_dataGridder(pplat_portion,pplong_portion,vTEC_portion,gif_Grid_Lat,gif_Grid_Long,gif_Grid_Lat_Spaces,gif_Grid_Long_Spaces,gif_Grid_Lat_Delta,gif_Grid_Long_Delta,dataRejectOrig,dataRejectLimitOrig,dataRejectMax):

    gif_Grid = np.nan*np.ones( (gif_Grid_Long_Spaces+1,gif_Grid_Lat_Spaces+1) ); #preallocate vTEC grid
            
    for j in prange(0,gif_Grid_Lat_Spaces): #gets the vTEC in the right area
        for l in range(0,gif_Grid_Long_Spaces):
