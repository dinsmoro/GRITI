...
def GRITI_import_ISR_Pokerflat(dateRange_dayNum_full,folder,dateRange_dayNum_zeroHr,pointAltitude,filter_cutoffPeriod = 2):
    ...
    for k in range(0,filenamePossibles.size):
        ...
        filenamePath = np.asscalar(filenamePossibles[filenameInRange[k]]); #get the current file path
        ...
        for j in range(0, PFISR_beamIDNum):
            ...
            try:
                PFISR_SNR_bp_tmp[j] = signal.filtfilt(b,a,PFISR_SNR_tmp[j],axis=1,padtype = 'odd', padlen=3*(b.size-1)); #Appies the filter
            ...
            #END TRY
            try:
                PFISR_POPL_bp_tmp[j] = signal.filtfilt(b,a,PFISR_POPL_tmp[j],axis=1,padtype = 'odd', padlen=3*(b.size-1)); #Appies the filter
                #extra bits from https://dsp.stackexchange.com/questions/11466/differences-between-python-and-matlab-filtfilt-function
            except ValueError:
        ...
        for j in range(0, PFISR_beamIDNum):
            tmp_cutOut = (np.int64(PFISR_time_tmp[j]) < dateRange_dayNum_full[0,1]) | (np.int64(PFISR_time_tmp[j]) > dateRange_dayNum_full[-1,1]); #get times out of bounds
    ...
    #            mainBeamIDIndex = np.where( np.isclose(mainElvMedian,np.median(PFISR_el_tmp[j])) )[0]; #connect where the data should go
                ...
                if( mainBeamIDIndex.size == 1 ):
                    mainBeamIDIndex = np.asscalar(mainBeamIDIndex); #convert to scalar so it's easy
