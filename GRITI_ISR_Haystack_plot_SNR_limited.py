"""
GOAL: Plot only ISR SNR limited RTI
RD on 4/11/19
...
def GRITI_ISR_Haystack_plot_SNR_limited(Zenith_time,Zenith_height,Zenith_SNR,MISA_time,MISA_height,MISA_SNR,ISR_cutoffPeriod,ISR_RTI_heightLimValues,ISR_plotLimValu,dateRange,dateRange_dayNum,dateRange_dayNum_zeroHr,FONT_titleFM,FONT_axisTick,FONT_axisLabelFM):
    #-----Plot ISR SNR results as a RTI-----
    #Plot just the ISR SNR results
    ...
    pltHelprX, pltHelprY = np.meshgrid( (Zenith_time - dateRange_dayNum_zeroHr[1])*24, \
                Zenith_height);
    im = ax[0].pcolormesh(pltHelprX , pltHelprY , Zenith_SNR , vmin=-ISR_plotLimValu , vmax=ISR_plotLimValu , cmap='gray' , shading='gouraud'); # pseudocolor plot "stretched" to the grid
    ...
    string_Title = 'Zenith SNR - No Filters, Amplitude Limited'; #create mecha title
    ...
    pltHelprX, pltHelprY = np.meshgrid( (MISA_time - dateRange_dayNum_zeroHr[1])*24, \
                MISA_height);
    im = ax[1].pcolormesh(pltHelprX , pltHelprY , MISA_SNR , vmin=-ISR_plotLimValu , vmax=ISR_plotLimValu , cmap='gray' , shading='gouraud'); # pseudocolor plot "stretched" to the grid
    cbar = fig.colorbar(im, cax=cax, orientation='vertical'); #create a colorbar using the prev. defined cax
    cbar.mappable.set_clim(vmin=-ISR_plotLimValu, vmax=ISR_plotLimValu);
    #cbar.ax.set_yticklabels(np.round(np.linspace(-ISR_plotLimValu,ISR_plotLimValu,5), len(str(ISR_plotLimValu).split('.')[1])+1 )); #create useful tick marks
    #cbar.set_label("SNR (unitless)"); #tabel the colorbar
    ...
    string_Title = 'MISA SNR - No Filters, Amplitude Limited'; #create mecha title
