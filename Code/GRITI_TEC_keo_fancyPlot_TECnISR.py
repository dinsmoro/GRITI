...
def GRITI_TEC_keo_fancyPlot_TECnISR(vTECChunked_anyAngleAvg,TEC_timeUnique,TEC_plotLimValu, \
    ...
    print('MAKING FANCY PLOT: TEC_avgAnyAngle_plot_wISR IN fancyPlot FOLDER'); #report since you won't see anything
    ...
    #ZENITH STUFF
    pltHelprX, pltHelprY = np.meshgrid( (Zenith_time - dateRange_dayNum_zeroHr[1]*86400)/3600, Zenith_height);
    im1 = ax[1].pcolormesh(pltHelprX , pltHelprY , Zenith_POPL_hp , vmin=np.min(ISR_POPL_plotLimValu) , vmax=np.max(ISR_POPL_plotLimValu), \
    ...
    cbar1.set_ticks(cbar1.get_ticks()); #get rid of tick shennanigans
    ...
    #         (np.round((np.max(Zenith_time)-dateRange_dayNum_zeroHr[1]*86400)/3600) - np.mod(np.round((np.max(Zenith_time)-dateRange_dayNum_zeroHr[1]*86400)/3600),2)) + 4 , \
    ...
    fig.savefig(folder[3]+'TEC_avgKeo&ISR.png'); #save the figure
