...
def subfun_Xcorrelator(sig1, sig2, sig1_filt=None, sig2_filt=None, sig1_noise=None, sig2_noise=None, time1=None, time2=None, time2shift=None, \
    ...
    if( FLG_plot == True):
        ...
        if( FLG_plot_onlyXcorr == False ):
            ...
            if( FLG_fancyPlot != 0 ):
                fig.savefig(ospath.join(settings_paths['fancyPlots'],'Xcorr_'+mode+plotName_fileName.replace(' ','+')+'_timeSeries'+'.png')); #save the figure
            ...
            figFitter(fig); #fit the fig fast
            if( FLG_fancyPlot != 0 ):
                fig.savefig(ospath.join(settings_paths['fancyPlots'],'Xcorr_'+mode+plotName_fileName.replace(' ','+')+'_FFT'+'.png')); #save the figure
                plt.close(); #close figure b/c it lurks apparently
                plt.ion(); #re-enable it for later stuff
            #END IF
        ...
        figFitter(fig); #fit the fig fast
        if( FLG_fancyPlot != 0 ):
            fig.savefig(ospath.join(settings_paths['fancyPlots'],'Xcorr_'+mode+plotName_fileName.replace(' ','+')+'_Xcorr'+'.png')); #save the figure
            plt.close(); #close figure b/c it lurks apparently
            plt.ion(); #re-enable it for later stuff
        #END IF
