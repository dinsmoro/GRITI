...
from shutil import make_archive, unpack_archive, move as shutil_move #backup in case 7zip no go go
...
def GRITI_import_AMPERE_direct(dates,settings_paths,AMPERE_hemi,settings_config,
    ...
    folder_fileNameFormat.append('ampere_#YR#MO#DY_#HEMI_grd.nc.zip'); #4 - expected file name style
    folder_fileNameFormat.append('ampere.#YR#MO#DY.k060_m08.#HEMI.grd.nc'); #5 - expected file name style
    folder_fileNameFormat.append('ampere.#YR#MO#DY.k060_m08.#HEMI.grd.nc.7z'); #6 - expected file name style
    folder_fileNameFormat.append('ampere.#YR#MO#DY.k060_m08.#HEMI.grd.nc.gz'); #7 - expected file name style
    folder_fileNameFormat.append('ampere.#YR#MO#DY.k060_m08.#HEMI.grd.nc.zip'); #8 - expected file name style
    folder_fileNameFormat.append('#YR#MO#DY.0000.86400.120.#HEMI.grd.ncdf'); #9 - expected file name style [old version of AMPERE data files]
        ...
        if( (AMPERE_sourceIndex >= 1) | (FLG_append == True) ): #this means it is the first type that has been refactored into an HDF5 file 
            ...
            if( FLG_directRead == True ):
                ...
                if( (AMPERE_fileName[AMPERE_fileName.rfind('.'):] == '.gz') | \
                    ...
                    (AMPERE_fileName[AMPERE_fileName.rfind('.'):] == '.zip') ):
                    #must unzip if so
                    try:
                        ...
                        if( AMPERE_fileName[AMPERE_fileName.rfind('.'):] != '.7z' ):
                            try:
                                unpack_archive(os.path.join(settings_paths['data'],folder_AMPERE,str(dateRange_full[i,0]),AMPERE_fileName), os.path.join(settings_paths['data'],folder_AMPERE,str(dateRange_full[i,0])));
                            except Exception as err2:
                                print('ERROR in GRITI_import_AMPERE_direct: 7zip filed to run. Here is 7zip-related error:\n'+str(err)+'\n THEN shutil.unpack_archive failed to ruin (bjorked file? never seent before so idk). Crashing, sorry. Here is that error:\n'+str(err2));
                                import sys
                                sys.crash();
                            #END TRYING
                        else:
                            ...
                            if( importlib.find_loader('py7zr') ):
                                ...
                                os.chdir(os.path.join(settings_paths['data'],folder_AMPERE,str(dateRange_full[i,0]))); #gotta move it cause can't tell it where to go?????
                                with py7zr.SevenZipFile(os.path.join(settings_paths['data'],folder_AMPERE,str(dateRange_full[i,0]),AMPERE_fileName), mode='r') as archive:
                                ...
                                print('ERROR in GRITI_import_AMPERE_direct: 7zip path of "'+settings_paths['7zip']+'" crashes. Maybe 7zip isn\'t there, maybe something else. py7zr also not installed. Crashing because file ending is .7z and needs 7zip. Here is the error:\n'+str(err));
                                import sys
                                sys.crash();
                            #END IF
                        #END IF
                ...
                try:
                    ...
                    if( importlib.find_loader('py7zr') ):
                        import py7zr
                        print('WARNING in GRITI_import_AMPERE_direct: 7zip compression failed, falling back to py7zr which is less compressed than the 7zip binary (but more than built-in zipping). Error follows: '+str(err));
                        curr_dir = os.getcwd(); #prep to revert
                        os.chdir(os.path.join(settings_paths['data'],folder_AMPERE,str(dateRange_full[i,0]))); #gotta move it cause can't tell it where to go?????
                        with py7zr.SevenZipFile(os.path.join(settings_paths['data'],folder_AMPERE,str(dateRange_full[i,0]),AMPERE_fileName)+'.7z', mode='w') as archive:
                            archive.write(os.path.join(settings_paths['data'],folder_AMPERE,str(dateRange_full[i,0]),AMPERE_fileName)); #crunch the file om nom nom
                        #END WITH
                        os.chdir(curr_dir); #reset to original, easier than moving the file b/c I'd have to figure out the file name or something idk
                    else:
                        print('WARNING in GRITI_import_AMPERE_direct: 7zip compression failed, falling back to built-in zipping which is less compressed. Error follows: '+str(err));
                        make_archive_better(os.path.join(settings_paths['data'],folder_AMPERE,str(dateRange_full[i,0]),AMPERE_fileName)+'.zip'); #call this function cause wow the call is bad and poorly explained with no examples (classic python docs)
                    #END IF
                #END TRYING
...
def make_archive_better(zip_file2zip_Wzip): #inspired by https://stackoverflow.com/a/50381250
    #zips a file (or maybe a folder, idk yolo there) into the folder it's already in - you prob need the full path with the file name for this to work
    #you need something like 'E:\\Big Data\\AMPERE\\2017\\AMPERED_2017_072_south.h5.zip' as the zip_file2zip_Wzip - the code'll figure out to zip it from .zip and figure out the file to zip from the rest.
    zip_file2zip_Wpath, zip_format = os.path.splitext(zip_file2zip_Wzip); #get the original file name and the format to zip with
    zip_format = zip_format[1:]; #remove the .
    zip_path = os.path.dirname(zip_file2zip_Wpath); #get the path to zip into (it doesn't anyway)
    zip_file2zip = os.path.basename(zip_file2zip_Wpath); #get the file name to zip
    curr_dir = os.getcwd(); #record for reverting
    os.chdir(zip_path); #gotta move it cause can't tell it where to go?????
    make_archive(zip_file2zip, zip_format, zip_path, zip_file2zip); #call make_archive's really convoluted calling method
    os.chdir(curr_dir); #revert to orig dir
#END DEF
