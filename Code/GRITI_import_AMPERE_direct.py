                ...
                if( (AMPERE_fileName[AMPERE_fileName.rfind('.'):] == '.gz') | \
                    ...
                    try:
                        ...
                        if( AMPERE_fileName[AMPERE_fileName.rfind('.'):] != '.7z' ):
                            ...
                            print('ERROR in GRITI_import_AMPERE_direct: 7zip path of "'+settings_paths['7zip']+'" crashes. Maybe 7zip isn\'t there, maybe something else. Crashing because file ending is .7z and needs 7zip. Here is the error:\n'+str(err));
                            import sys
                            sys.crash();
                        #END IF
                    #END TRYING
...
def make_archive_better(zip_file2zip_Wzip): #inspired by https://stackoverflow.com/a/50381250
    ...
    zip_file2zip_Wpath, zip_format = os.path.splitext(file2zipWzip); #get the original file name and the format to zip with
    zip_format = zip_format[1:]; #remove the .
    zip_path = os.path.dirname(zip_file2zip_Wpath); #get the path to zip into
    ...
    shutil_move(zip_file2zip+'.'+zip_format, zip_file2zip_Wzip); #gotta move it cause can't tell it where to go?????
#END DEF
