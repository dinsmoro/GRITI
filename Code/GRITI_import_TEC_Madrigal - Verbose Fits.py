...
#dateRange = np.array([[5,7,2013],[5,7,2013]],dtype="int16"); #dates are in int16 because they can be
dateRange = np.array([[7,31,2014],[7,31,2014]],dtype="int16"); #dates are in int16 because they can be
#dateRange = np.array([[12,31,2014],[1,1,2015]],dtype="int16"); #for debug, check year success
...
minElevation = 50; #deg, min elevation angle accepted
...
print("\nDate range requested (day num/yr format): {}/{} to {}/{}.".format(dateRange_dayNum_full[0,0],dateRange_dayNum_full[0,1],dateRange_dayNum_full[-1,0],dateRange_dayNum_full[-1,1]) )
...
if( os.path.isdir(folder[1] + '\\' + folder_TEC) == 0 ): #check if TEC folder exists
    ...
    print("NOTA BENE: Importing TEC - Created TEC directory: {}\n".format(folder[1] + '\\' + folder_TEC) );
#END IF
    
dateRange_uniqueYears = np.unique(dateRange_dayNum_full[:,1]); #get the unique years involved
...
TEC_dataPath = ["{}\{}\{}".format(a_, b_, c_) for a_, b_, c_ in zip([folder[1]]*TEC_dataAmnt, [folder_TEC]*TEC_dataAmnt,np.ndarray.tolist(dateRange_dayNum_full[:,1].astype(str)) ) ]; #get the base path where data will be in
TEC_dataFileName = ["{}_{}_{}{}".format(a_, b_, c_, d_) for a_, b_, c_, d_ in zip([folder_TEC]*TEC_dataAmnt,np.ndarray.tolist(dateRange_dayNum_full[:,1].astype(str)),np.ndarray.tolist(dateRange_dayNum_full[:,0].astype(str)),[folder_fileEnding]*TEC_dataAmnt ) ]; #get the expected filenames
TEC_dataFileNameUnfilt = ["{}_{}_{}_unfilt{}".format(a_, b_, c_, d_) for a_, b_, c_, d_ in zip([folder_TEC]*TEC_dataAmnt,np.ndarray.tolist(dateRange_dayNum_full[:,1].astype(str)),np.ndarray.tolist(dateRange_dayNum_full[:,0].astype(str)),[folder_fileEnding]*TEC_dataAmnt ) ]; #get the expected filenames for unfiltered data (if stopped mid filtering)
...
for i in range(0,len(dateRange_dayNum_full[:,0])): #loop to check if any data exists - data will not be downloaded yet!
    ...
    if( os.path.isfile(TEC_dataFilePath[i]) == 0):
        ...
        TEC_fileNameOrig = 'los_' + str(dateRange_full[i,2]) + str(dateRange_full[i,0]).zfill(2) + str(dateRange_full[i,1]).zfill(2) + '.001.h5'; #create expected file name 
        
        if( (os.path.isfile(folder[1] + '\\' + folder_TEC + '\\' + str(dateRange_full[i,2]) + '\\' + TEC_fileNameOrig) == 0) and (os.path.isfile(TEC_dataFilePathUnfilt[i]) == 0) ):
            #if the unconverted file is not there, download it
            #first - need to check if TEC data is availiable for the days requested
            
            if( FLG_newYr != dateRange_full[i,2] ): #check to go get the year's web links and file names and file dates
                #basically, if within the same year only need to get this once for the whole year's data availiability
                FLG_newYr = dateRange_full[i,2]; #set it so the flag works good
            
                web_fileSearch = web_base_site + web_base + str(dateRange_dayNum_full[i,1]) + web_baseAfter; #build site to go to
            ...
            if( np.any(np.all(( web_fileNamesDate - dateRange_full[i,:] )  == 0, axis=1 ) == 1 )): #basically checks the date vector with all the dates on the website, if one matches we're good
                ...
                if( np.all(np.any(dateRange_dayNum_full_orig == dateRange_dayNum_full[i,:],axis=0)) == 1 ): 
                    ...
                    print("\n==============ERROR==============");
                    print("There is no data availiable on {}/{}/{} in M/D/YR format. Printing availiable data days for the relevant year - will exit on finishing checking all days:".format(dateRange_full[i,0],dateRange_full[i,1],dateRange_full[i,2]) );
            ...
            try: #gonna try to read the file - if we fail, it failed mid download probably
                ...
                try: #gonna try to read the file - if we fail, it failed mid download
                    with h5py.File(folder[1] + '\\' + folder_TEC + '\\' + str(dateRange_full[i,2]) + '\\' + TEC_fileNameOrig, 'r') as testFile:
                        testFile.keys(); #tries to check out some stuff in the file
                    #END WITH
                    ...
                    print("\n==============~Warning~==============");
                    print("Data wasn't downloaded fully for {} and will be redownloaded. Deleting partial file as well.\n".format(folder[1] + '\\' + folder_TEC + '\\' + str(dateRange_full[i,2]) + '\\' + TEC_fileNameOrig) );
                    os.remove(folder[1] + '\\' + folder_TEC + '\\' + str(dateRange_full[i,2]) + '\\' + TEC_fileNameOrig); #delete partially downloaded file to avoid conflicts
...
for i in range(0,len(dateRange_dayNum_full[:,0])): #to download and unpack OR just unpack needed data
    ...
    if( np.all(np.any(dateRange_dayNum_full_orig == dateRange_dayNum_full[i,:],axis=0)) == 1): #check for original data requested
        print("\n==============Now Working on Date: {}/{}/{} (M/D/Y) or {}/{} (D#/Y)==============".format(dateRange_full[i,0],dateRange_full[i,1],dateRange_full[i,2],dateRange_dayNum_full[i,0],dateRange_dayNum_full[i,1]) );
    else: #else it is a padded day - no filtering will occur
        print("\n==============Now Working on Date: {}/{}/{} (M/D/Y) or {}/{} (D#/Y) - Padded Date, Filter will be Skipped==============".format(dateRange_full[i,0],dateRange_full[i,1],dateRange_full[i,2],dateRange_dayNum_full[i,0],dateRange_dayNum_full[i,1]) );
    #END IF
    ...
    if( TEC_dataAvail[i] == 2 ): #2 means data needs to be downloaded
        ...
        print("Downloading {} to {}.\nFile size is {:.2f} GB. At 10 MB/s (80 Mbps) expect it to take {:.2f} min.\nFrom site {}".format( current_webFileNames,folder[1] + '\\' + folder_TEC + '\\' + str(dateRange_full[i,2]), current_webFileSize/(1024**3),(current_webFileSize/1024**2*(1/10))/60, web_base_site+current_web_fileLinks ));
        tic = time.time(); #for time testing
        urlretrieve(web_base_site+current_web_fileLinks,folder[1] + '\\' + folder_TEC + '\\' + str(dateRange_full[i,2]) + '\\' + current_webFileNames,reporthook=downloadProgress); #download the file in question to the data directory
    ...
    if( TEC_dataAvail[i] == 3 ): #3 means data was downloaded but needs to be converted
        ...
        with h5py.File(folder[1] + '\\' + folder_TEC + '\\' + str(dateRange_full[i,2]) + '\\' + TEC_fileNameOrig, 'r') as TEC_fileOrig:
            #TEC_fileOrig = h5py.File(folder[1] + '\\' + folder_TEC + '\\' + str(dateRange_full[i,2]) + '\\' + TEC_fileNameOrig, 'r'); #read that file
        ...
        #use the with thing because it prevents the file being "open in python" still in the event of a crash THERE IS NO WAY TO CLOSE A FILE?
        with h5py.File(folder[1] + '\\' + folder_TEC + '\\' + str(dateRange_full[i,2]) + '\\' + TEC_dataFileNameUnfilt[i], 'w') as TEC_fileUnfilt:
            #TEC_fileUnfilt = h5py.File(folder[1] + '\\' + folder_TEC + '\\' + str(dateRange_full[i,2]) + '\\' + TEC_dataFileNameUnfilt[i], 'w'); #write that file new file
            ...
            sys.stdout.flush();
        #END WITH
        os.remove(folder[1] + '\\' + folder_TEC + '\\' + str(dateRange_full[i,2]) + '\\' + TEC_fileNameOrig); #delete the downloaded file now that we're done with it (save some space!)
    ...
    if( (TEC_dataAvail[i] > 4) or (TEC_dataAvail[i] < 0) ): #otherwise we got problems!
        print("\n==============ERROR==============");
        print("TEC_dataAvail was set to {} on {}/{}/{} in M/D/YR format. That's not 0, 1, 2, 3, or 4 so something real wrong. Exiting.".format(TEC_dataAvail[i],dateRange_full[i,0],dateRange_full[i,1],dateRange_full[i,2]) );
        #return("No."); #Can I? I shall.
    #END IF  
...
for i in range(0,len(dateRange_dayNum_full[:,0])): #to download and unpack OR just unpack needed data
        ...
        if( np.all(np.any(dateRange_dayNum_full_orig == dateRange_dayNum_full[i,:],axis=0)) == 1 ): #if statement to find original days requested
            ...
            for j in range(-1,2): #run through the days to get the main day and the days around it
                if(TEC_dataAvail[i+j] == 4): #make sure data is there before reading it
                    with h5py.File(folder[1] + '\\' + folder_TEC + '\\' + str(dateRange_full[i,2]) + '\\' + TEC_dataFileNameUnfilt[i+j], 'r') as TEC_fileUnfilt:
                        #-----import floats (big boys)-----
                        #this is done in waves to keep the memory usage under control
                        TEC_fileData_float_temp = TEC_fileUnfilt.get("float").value;
                        ...
                        #this is done in waves to keep the memory usage under control
                        TEC_fileData_int_temp = TEC_fileUnfilt.get("int").value; #grab that data
                        ...
                        #this is done in waves to keep the memory usage under control
                        TEC_fileData_string_temp = TEC_fileUnfilt.get("string").value;
                    ...
                    try: #padding warning
                        if( j == -1 ): #before padding not there
                            ...
                            print("\n==============ERROR==============");
                            print("There's no data on a main day requested {}/{} - which shouldn't have happened. Help. Exiting.\n".format(dateRange_dayNum_full[i,0],dateRange_dayNum_full[i,1]) );
                            #return("No");
                        #END IF
                    except(NameError):
                        if( j == -1 ): #before padding not there
            ...
            #for j in range(0,np.int64(len(TEC_fileData_string_unique)/4)): #run through all of the sites
                ...
                #gg = TEC_fileData_float[currentSite_loc,1];
                #gg2 = TEC_fileData_float[6000:6100,1];
                
    #            plt.figure();
    #            plt.scatter( TEC_fileData_float[currentSite_loc,1] , TEC_fileData_float[currentSite_loc,5] );
    #            plt.title("Data for Site "+TEC_fileData_string_unique[j].decode('UTF-8'));
    #            plt.xlabel('Time [days]');
    #            plt.ylabel('los TEC [TECU]');
    #            plt.show();
    #            xlim_lower, xlim_upper = plt.xlim(); #get the current x limits
    #            ylim_lower, ylim_upper = plt.ylim(); #get the current y limits
        ...
        #            plt.figure();
        #            plt.scatter( currentDayNum , currentTEC );
        #            plt.title("Data for Site "+TEC_fileData_string_unique[j].decode('UTF-8')+" and sat "+str(currentSat_unique[k]));
        #            plt.xlabel('Time [days]');
        #            plt.ylabel('los TEC [TECU]');
        #            plt.xlim(xlim_lower, xlim_upper); #set the y limits
        #            plt.ylim(ylim_lower, ylim_upper); #set the y limits
        #            plt.show();
                    ...
                    #gg1 = currentDayNum[currentDayNum_timeSplits_loc[0]:(currentDayNum_timeSplits_loc[1])];
                    #gg2 = currentDayNum[currentDayNum_timeSplits_loc[1]:(currentDayNum_timeSplits_loc[2])];
                    #gg3 = currentDayNum[currentDayNum_timeSplits_loc[2]:(currentDayNum_timeSplits_loc[3])];
                    ...
                    #l = 0;
                        ...
                        if( (currentDayNum[currentDayNum_timeSplits_loc[l+1]-1]-currentDayNum[currentDayNum_timeSplits_loc[l]])*1440 > filterPeriod*2 ):
                            ...
                            #doing this instead, it's a spline but it's quad so there's a polynomial in there so it is named right
        #                    currentPolyCoef = interpolate.splrep(currentDayNum_singleSatLine_min, currentTEC_singleSatLine,k=2,s=np.mean(currentTECerror_singleSatLine)*1000); #fit a spline, smooth based on the mean of the TEC (it seemed like a good value - based around a median TEC value)
        #                    currentPolyYvals = interpolate.splev(currentDayNum_singleSatLine_min, currentPolyCoef); #calculate the median values
                            
        #                    plt.figure();
        #                    plt.scatter( range(0,len(currentTEC_singleSatLine)) , currentTEC_singleSatLine , 20 , "r" );
        #                    plt.scatter( range(0,len(currentTEC_singleSatLine)) , currentPolyYvals , 20 );
        #                    plt.title("1st Guess - Data for Site "+TEC_fileData_string_unique[j].decode('UTF-8')+" and sat "+str(currentSat_unique[k])+" and "+str(l+1)+" track");
        #                    plt.xlabel('Index');
        #                    plt.ylabel('los TEC [TECU]');
        #                    plt.show();
        ...
        #                    weight_estimatedLocalMin = np.int64(np.round(np.mean(argrelmin(currentPolyYvals)[0]))); #calc local minimum of guessed data
                            ...
                            #adapted from answer https://stackoverflow.com/questions/19791581/how-to-use-leastsq-function-from-scipy-optimize-in-python-to-fit-both-a-straight @Robert Ribas
                            #currentFun is a "lambda" function that has the polynomial stuff in it
                            #currentPolyCoef is a tuple that contains the parameters of the fit
                            #currentFun = lambda currentPolyCoef, currentDayNum_singleSatLine_min : currentPolyCoef[0] + currentPolyCoef[1]*(currentDayNum_singleSatLine_min-currentDayNum_singleSatLine_min[weight_estimatedLocalMin]) + ( (currentPolyCoef[2]*(currentDayNum_singleSatLine_min-currentDayNum_singleSatLine_min[weight_estimatedLocalMin])) - np.abs(currentPolyCoef[3]*(currentDayNum_singleSatLine_min-currentDayNum_singleSatLine_min[weight_estimatedLocalMin])) )**2; #create a "lambda" function whatever that is that has a tuple for the polynomial coefficients in i
                            #currentFun = lambda currentPolyCoef, currentDayNum_singleSatLine_min : currentPolyCoef[0] + currentPolyCoef[1]*(currentDayNum_singleSatLine_min-currentDayNum_singleSatLine_min[weight_estimatedLocalMin]) + currentPolyCoef[2]*(currentDayNum_singleSatLine_min-currentDayNum_singleSatLine_min[weight_estimatedLocalMin])**2 + currentPolyCoef[3]*(currentDayNum_singleSatLine_min-currentDayNum_singleSatLine_min[weight_estimatedLocalMin])**4; #create a "lambda" function whatever that is that has a tuple for the polynomial coefficients in it
                            
                            #currentFun = lambda coef, x : coef[0] + coef[1]*x + coef[2]*x**2 ; #create a "lambda" function whatever that is that has a tuple for the polynomial coefficients in it
        #                    #currentErrorFun is the diference between the currentFun and the y "experimental" data + currentPolyCoef[3]*currentDayNum_singleSatLine_min**4
                            #currentErrorFun = lambda coef, x, y: currentFun(coef,x) - y; #create a "lambda" function for the error between the real data y (currentTEC_singleSatLine) and the fit data y (currentFun)
        #                    
        #                    #currentPolyCoef_init contains the first guess of the currentPolyCoef polynomial coefficients
        #                    #gg = np.min(currentTEC_singleSatLine);
        #                    currentPolyCoef_init = (np.min(currentTEC_singleSatLine), -1.58701326, 0.001011, currentDayNum_singleSatLine_min[np.where( np.min(currentTEC_singleSatLine) == currentTEC_singleSatLine)[0][0] ] ); #this is a tuple apparently, first guess
        #                    currentPolyCoef_init = (360, -14, .0016, .000058 ); #this is a tuple apparently, first guess
                            #currentPolyCoef_init = (np.min(currentTEC_singleSatLine)*10, -1.58701326*10, 0.1011);
                            #currentPolyCoef_init = (250, -10, 0.2, 0.2);
                            #currentPolyCoef_init = (25, -1, 0.2, 0.2); #for abs model
        ...
        #                    
        #                    #least_squares finds the set of parameters in the tuple currentPolyCoef that minimizes
        #                    currentPolyCoef = least_squares(currentErrorFun,currentPolyCoef_init, args=(currentDayNum_singleSatLine_min,currentTEC_singleSatLine) ); #calculate the polynomial coefficients to fit the data
                            #currentPolyCoef = minimize(funErrorHlpr,currentPolyCoef_init, args=(currentDayNum_singleSatLine_min,currentTEC_singleSatLine),method="Nelder-Mead" ); #calculate the polynomial coefficients to fit the data
                            ...
                            #currentPolyCoef = fmin(funErrorHlpr,currentPolyCoef_init, args=(currentDayNum_singleSatLine_min,currentTEC_singleSatLine) ); #ca
                            #currentPolyYvals = currentFun(currentPolyCoef,currentDayNum_singleSatLine_min); #calculate the polynomial that was fit to the data
                            
    #                        plt.figure();
    #                        plt.scatter( currentDayNum_singleSatLine , currentElv_singleSatLine , 20 , "r" );
    #                        plt.title("Elv - Data for Site "+TEC_fileData_string_unique[j].decode('UTF-8')+" and sat "+str(currentSat_unique[k])+" and "+str(l+1)+" track");
    #                        plt.xlabel('Time [days]');
    #                        plt.ylabel('elv [deg]');
    #                        plt.show();
                            
        #                    plt.figure();
        #                    plt.scatter( currentDayNum[currentDayNum_timeSplits_loc[l]:currentDayNum_timeSplits_loc[l+1]] , currentTEC[currentDayNum_timeSplits_loc[l]:currentDayNum_timeSplits_loc[l+1]] , 20 , "r" );
        #                    plt.scatter( currentDayNum_singleSatLine , currentPolyYvals , 20 );
        #                    plt.title("Powell Mini - Data for Site "+TEC_fileData_string_unique[j].decode('UTF-8')+" and sat "+str(currentSat_unique[k])+" and "+str(l+1)+" track");
        #                    plt.xlabel('Time [days]');
        #                    plt.ylabel('los TEC [TECU]');
        #                    plt.show();
        #                    
        ...
        #                    plt.title("Polyfit - Data for Site "+TEC_fileData_string_unique[j].decode('UTF-8')+" and sat "+str(currentSat_unique[k])+" and "+str(l+1)+" track");
        #                    plt.xlabel('Time [days]');
        #                    plt.ylabel('los TEC [TECU]');
        #                    plt.show();
                                                
        #                    currentPolyCoef = np.polynomial.polynomial.polyfit(currentDayNum_singleSatLine_min, currentTEC_singleSatLine, 4); #gets the coefficients that fit an x^2 polynomial - what a function to call am I right
        #                    currentPolyCoef_firstDeriv = [currentPolyCoef[cntr]*cntr for cntr in range(1, len(currentPolyCoef))]
        #                    
        #                    currentFunRootzFinder = lambda rootz : currentPolyCoef_firstDeriv[0] + currentPolyCoef_firstDeriv[1]*rootz + currentPolyCoef_firstDeriv[2]*rootz**2 + currentPolyCoef_firstDeriv[3]*rootz**3; 
        #                    currentRootz = fsolve(currentFunRootzFinder, (np.mean(currentDayNum_singleSatLine_min)*100,np.mean(currentDayNum_singleSatLine_min),-np.mean(currentDayNum_singleSatLine_min)*100) ); #find all 3 roots
        #                    
                            
                            #first deriv of original function
                            #xr_guess = (np.mean(currentDayNum_singleSatLine_min)*100,np.mean(currentDayNum_singleSatLine_min),-np.mean(currentDayNum_singleSatLine_min)*100);
                            #currentFunRootzFinderFirst = lambda coef, xr : coef[1] + 3*(coef[3]**3)*(xr - coef[5])**2 + 4*(np.abs(coef[4])**4)*(xr - coef[5])**3 + 2*(np.abs(coef[2]) - ((coef[6]**2)*(xr-coef[5]))/np.abs(coef[6]*(xr-coef[5])))*(np.abs(coef[2])*(xr-coef[5]) - np.abs(coef[6]*(xr - coef[5]))); 
                            #currentRootzFirst = minimize(funErrorHlprFirst,xr_guess,args=(currentPolyCoef.x),method="Powell" ); #calculate the polynomial coefficients to fit the data
    #                        
    #                        currentRootzFirst = fsolve(currentFunRootzFinderFirst, (np.mean(currentDayNum_singleSatLine_min)*100,np.mean(currentDayNum_singleSatLine_min),-np.mean(currentDayNum_singleSatLine_min)*100) ); #find all 3 roots
    #                        
    #                        #second deriv of original function
                            #xrr_guess = (np.mean(currentDayNum_singleSatLine_min)*10,np.mean(currentDayNum_singleSatLine_min),-np.mean(currentDayNum_singleSatLine_min)*10);
                            ...
                            #currentRootzSecond = fsolve(currentFunRootzFinderFirst, (np.mean(currentDayNum_singleSatLine_min)*100,np.mean(currentDayNum_singleSatLine_min),-np.mean(currentDayNum_singleSatLine_min)*100) ); #find all 3 roots
                            #currentRootzSecond = minimize(funErrorHlprSecond,xrr_guess,args=(currentPolyCoef.x), method="Powell" ); #calculate the polynomial coefficients to fit the data
                            
                            
        #                    if( (np.sum((currentRootz > np.min(currentDayNum_singleSatLine_min)) & (currentRootz < np.max(currentDayNum_singleSatLine_min))) > 1) | (np.sum((currentRootz > np.min(currentDayNum_singleSatLine_min)) & (currentRootz < np.max(currentDayNum_singleSatLine_min))) == 0) ):
        #                        #if x^3 coefficient is negative, revert to x^2 polynomial because that means it's gonna do an S and not a U
        #                        print("MULTIPLE ROOTS DETECTED IN GIVEN RANGE: for Site "+TEC_fileData_string_unique[j].decode('UTF-8')+" and sat "+str(currentSat_unique[k])+" and "+str(l+1)+" track")
        #                        print("{}\n\n".format(currentPolyCoef))
        #                        currentPolyCoef = np.polynomial.polynomial.polyfit(currentDayNum_singleSatLine_min, currentTEC_singleSatLine, 2); #gets the coefficients that fit an x^2 polynomial - what a function to call am I right
        #                    currentPolyYvals = np.polynomial.polynomial.polyval(currentDayNum_singleSatLine_min, currentPolyCoef); #calc the guessed values 
        #                        
                            ...
                            #currentConcavityVals = currentFun(currentPolyCoef.x,np.linspace(np.min(currentDayNum_singleSatLine_min),np.max(currentDayNum_singleSatLine_min),len(currentDayNum_singleSatLine_min)*10) ); #calculate the polynomial that was fit to the data
                            #currentConcavityVals = np.diff(currentConcavityVals); #calculate delta between the vectors
                            
                            #if 2nd deriv has negative values, then concavity is down which is never what we want
                            #this finds that
    #                        if( np.any(currentFunRootzFinderSecond(currentPolyCoef.x,currentDayNum_singleSatLine_min) < 0) ):
    #                            plt.figure();
    #                            plt.scatter( currentDayNum_singleSatLine_min , currentTEC_singleSatLine , 20 , "r" );
    #                            plt.scatter( currentDayNum_singleSatLine_min , currentPolyYvals , 20 );
    #                            plt.title("0th Deriv HERE Fit "+fitType+": Data for Site "+TEC_fileData_string_unique[j].decode('UTF-8')+" and sat "+str(currentSat_unique[k])+" and "+str(l+1)+" track");
    #                            plt.xlabel('Time [min]');
    #                            plt.ylabel('los TEC [TECU]');
    #                            plt.show();
    #                            
    #                            plt.figure();
    #                            plt.scatter( currentDayNum_singleSatLine_min , currentFunRootzFinderFirst(currentPolyCoef.x,currentDayNum_singleSatLine_min) , 20 , "r" );
    #                            plt.title("1st Deriv HERE Fit "+fitType+": Data for Site "+TEC_fileData_string_unique[j].decode('UTF-8')+" and sat "+str(currentSat_unique[k])+" and "+str(l+1)+" track");
    #                            plt.xlabel('Time [min]');
    #                            plt.ylabel('los TEC [TECU]');
    #                            plt.show();
    #                            
    #                            plt.figure();
    #                            plt.scatter( currentDayNum_singleSatLine_min , currentFunRootzFinderSecond(currentPolyCoef.x,currentDayNum_singleSatLine_min) , 20 , "r" );
    #                            plt.title("2nd Deriv HERE Fit "+fitType+": Data for Site "+TEC_fileData_string_unique[j].decode('UTF-8')+" and sat "+str(currentSat_unique[k])+" and "+str(l+1)+" track");
    #                            plt.xlabel('Time [min]');
    #                            plt.ylabel('los TEC [TECU]');
    #                            plt.show();
                            #END IF
    ...
    #                        plt.figure();
    #                        plt.scatter( currentDayNum_singleSatLine_min , currentTEC_singleSatLine , 20 , "r" );
    #                        plt.scatter( currentDayNum_singleSatLine_min , currentFunRootzFinderFirst(currentDayNum_singleSatLine_min) , 20 );
    ...
    #                        plt.show();
        ...
        #                        plt.scatter( currentDayNum_singleSatLine , currentPolyYvals_firstFit , 20 );
        #                        plt.title("Fit Good (polyfit ^4): Data for Site "+TEC_fileData_string_unique[j].decode('UTF-8')+" and sat "+str(currentSat_unique[k])+" and "+str(l+1)+" track");
        #                        plt.xlabel('Time [days]');
        #                        plt.ylabel('los TEC [TECU]');
        #                        plt.show();
        #                        
        ...
        #                        plt.scatter( currentDayNum_singleSatLine , currentTEC_singleSatLine , 20 , "r" );
        #                        plt.scatter( currentDayNum_singleSatLine , currentPolyYvals , 20 );
        #                        plt.title("Fit Bad (mecha function): Data for Site "+TEC_fileData_string_unique[j].decode('UTF-8')+" and sat "+str(currentSat_unique[k])+" and "+str(l+1)+" track");
        #                        plt.xlabel('Time [days]');
        #                        plt.ylabel('los TEC [TECU]');
        #                        plt.show();
        #                        
    ...
    #                                plt.scatter( currentDayNum_singleSatLine , currentTEC_singleSatLine , 20 , "r" );
    #                                plt.scatter( currentDayNum_singleSatLine , currentPolyYvals , 20 );
    #                                plt.title("REPLACED big eq - Data for Site "+TEC_fileData_string_unique[j].decode('UTF-8')+" and sat "+str(currentSat_unique[k])+" and "+str(l+1)+" track");
    #                                plt.xlabel('Time [days] '+"error: "+str(np.sum(current_deltaTEC**2)));
    #                                plt.ylabel('los TEC [TECU]');
    #                                plt.show();
    ...
    #                                plt.scatter( currentDayNum_singleSatLine , currentTEC_singleSatLine , 20 , "r" );
    #                                plt.scatter( currentDayNum_singleSatLine , currentPolyYvals , 20 );
    #                                plt.title("with this "+fitType+" - Data for Site "+TEC_fileData_string_unique[j].decode('UTF-8')+" and sat "+str(currentSat_unique[k])+" and "+str(l+1)+" track");
    #                                plt.xlabel('Time [days] '+"error: "+str(np.sum(current_deltaTEC_redo**2))+" compared to poly4 error: "+str(np.sum(current_deltaTEC_firstFit**2))+" or big eq error: "+str(np.sum(current_deltaTEC**2)));
    #                                plt.ylabel('los TEC [TECU]');
    #                                plt.show();
