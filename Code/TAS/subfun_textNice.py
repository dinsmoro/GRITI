...
# -*- coding: utf-8 -*-
"""
Makes numbers look nice, use in place of str() like textNice(np.round(somenumberthatcanbeanintegersometimes,3))
"""
...
def textNice(number):
    if( isinstance(number,str) ):
    ...
    elif( isinstance(number,ndarray) ):
         if( number.ndim != 0 ):
             text = '';
        ...
        if( isinstance(number,list) | isinstance(number,tuple) ):
            text = '';
            for i in range(0,len(number)):
                text = text + textNice_cleanser(str(number[i])) + ', ';
            #END FOR i
