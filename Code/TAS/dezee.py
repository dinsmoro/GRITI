...
# -*- coding: utf-8 -*-
"""
Unzips files that have the '.Z' extension, technically called 'LZW compression'
Use like:
---
import TAS.dezee
with dezee.open(file2read) as finput:
    file2read = finput.read(); # Read in-place, it's now BYTES (if text, convert to text with `file2read.decode('utf-8')`)
# END WITH
---
OR
---
...
with dezee.open(file2read) as finput:
    finput_read = finput.read(); # Read in-place, it's now BYTES
    with open(ospath.splitext(file2read)[0], 'wb') as foutput:
        foutput.write(finput_read); # Write the unzipped byte array as a FILE
    # END WITH
# END WITH
file2read = ospath.splitext(file2read)[0]; # Get the unzipped file name to read in
---
"""
from os import PathLike as osPathLike

def open(file2read):
    return dezee(file2read); # Get it
# END DEF
...
class dezee:
    ...
    def read(self):
        # --- Ensure the file is bytes ---
        if( not isinstance(self.file2read, bytes) ): # Want bytes, if not try to open the file
            # ! if you send a byte string in (b'/im/a/path.Z') it's not gonna know it's a byte string, decode it first (`file2read.decode('utf-8')`)
            if( isinstance(self.file2read, (str, osPathLike)) ):
                with open(self.file2read, 'rb') as finput:
                    self.file2read = finput.read(); # Read in-place, it's now BYTES
                # END WITH
            else:
                raise Exception('ERROR in unzip_z.dezee: Type of input file '+str(type(self.file2read))+' is not a bytes object OR a string/path.');
            # END IF
        # END IF
        
        # --- Decode the file, use https://github.com/scivision/unlzw3/blob/main/src/unlzw3/unlzw.py ---
        """
        ...
        if (self.file2read[0] != 0x1F) or (self.file2read[1] != 0x9D):
            raise ValueError("Invalid Header Flags Byte: Incorrect magic bytes")
        
        flags = self.file2read[2]
        ...
        buf = self.file2read[3]
        buf += self.file2read[4] << 8
        ...
        return bytes(bytearray(put)) # Return bytes
    # END DEF
