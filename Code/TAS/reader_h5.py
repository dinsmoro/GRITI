...
def reader_h5(h5_file, h5_path_prefix=None, diction=None, FLG_oneRead=False, FLG_singleListRemove=True, FLG_table2pandas=False):
    #--- prep work ---
    ...
    # if( not isinstance( h5_file, h5py._hl.files.File) ):
        ...
        with h5py.File( h5_file, 'r') as h5_file_reader:
            diction = reader_h5(h5_file_reader, h5_path_prefix=h5_path_prefix, diction=diction, FLG_oneRead=FLG_oneRead, FLG_singleListRemove=FLG_singleListRemove, FLG_table2pandas=FLG_table2pandas)
        # END WITH
    # END IF
        ...
        for key, item in h5_file.items():  
            ...
            if( isinstance(item, h5py.Dataset) ): #dataset catch
                ...
                if( any(['pandas_type' in tup[0] for tup in list(item.attrs.items())]) and any(['pandas_version' in tup[0] for tup in list(item.attrs.items())]) ):
                    ...
                    diction[key] = reader_h5(item, h5_path_prefix=None, diction=diction[key], FLG_oneRead=FLG_oneRead, FLG_singleListRemove=FLG_singleListRemove, FLG_table2pandas=FLG_table2pandas); #recurse
                # END IF
            #END IF
        #END FOR key, item
    # END IF
    ...
    if( FLG_singleListRemove ):
        ...
        for keyz in diction.keys():
            if( isinstance(diction[keyz], (list, tuple)) and (len(diction[keyz]) == 1) ):
