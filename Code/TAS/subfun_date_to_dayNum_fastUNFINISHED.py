...
#RD on 8/23/2018
# FAST VERSION LOSES SOME VERBOSENESS IN EXCHANGE FOR POWER
...
def nb_expand_dims( array ): # Pure python version for overload to link to
    if( array.ndim == 1 ):
...
@overload(nb_expand_dims)
def OVERLOADED_nb_expand_dims( array ):
    if( array.ndim == 1 ):
        return lambda array: np.expand_dims(array, axis=0)
    else:
        return lambda array: array
    # END IF
# END DEF

@jit(nopython=True, nogil=True, parallel=False, fastmath=True) #try to activate some parallel speed up stuff (doesn't work really) #,nogil=True, parallel=True, fastmath=True
def subfun_date_to_dayNum_fast(dateRange, options = 1):
...
#==============Catch input format issues==============
    if( (dateRange.ndim == 1) & (dateRange.size == 3) ):
        dateRange = dateRange.copy().reshape(1,dateRange.size); #prep for an array because numpy throws a hissy if you have a "vector" and not a "[vector,1] array" lol am I right
    else:
        if( (dateRange[0,:].size != 3) & (dateRange[:,0].size == 3) ):
...
#            dateRange = np.swapaxes(dateRange,0,1); #flip axes
            dateRange = dateRange.T; #flip axes
            
        elif (dateRange[0,:].size != 3) & (dateRange[:,0].size != 3):
...
#            print("Input size was ["+str(len(dateRange[0,:]))+","+str(len(dateRange[:,0]))+"}] and it needs to be [arbitrary,3] - exiting date to dayNum fun!\n"); #non-.format version for nopython calls
            raise Exception("Input size was ["+str(len(dateRange[0,:]))+","+str(len(dateRange[:,0]))+"}] and it needs to be [arbitrary,3] - exiting date to dayNum fun!\n");
        #END IF
    #END IF
...
#==============Convert dates given in M/D/YR format to dayNum/YR format==============    
    ...
    #-----Option 0 returns the default output of [dayNum/Yr] format-----
        ...
        dateRange_dayNum = np.zeros( (dateRange[:,0].size , 2 ), dtype=np.int16 ); #preallocate , numba happy
        tempMonth = np.int16(0); #prep to make numba happy
        dateRange_unique = np.unique(dateRange); #get the unique dates involved (helps if there are many repeats) 
        dateRange_num2do = dateRange_unique.size//3; # Get the number to do bypassing numba issues I hope
        if( dateRange_unique.ndim == 1 ):
            dateRange_unique = nb_expand_dims(dateRange_unique);
        # END IF
        
        for i in range(0, dateRange_num2do ):
            #-----Leap Year Detection-----
            if np.mod(dateRange_unique[i,0],4) == 0: #leap year
                #-----Leap Year Skipped Detected - next will be 2100-----
                if( (np.mod(dateRange_unique[i,0],100) == 0) & (np.mod(dateRange_unique[i,0],400) != 0) ):
    ...
    #-----Option 1 returns the output in the [Yr/dayNum] format-----
        #dateRange_dayNum = np.zeros( (len(dateRange[:,0]) , 2 ), dtype=np.int16 ); #preallocate 
        dateRange_dayNum = np.int16(np.zeros( (dateRange[:,0].size , 2 ) ) ); #preallocate, numba happy
        tempMonth = np.int16(0); #prep to make numba happy
        dateRange_unique = np.unique(dateRange); #get the unique dates involved (helps if there are many repeats) 
        if( dateRange_unique.ndim == 1 ):
            dateRange_unique = nb_expand_dims(dateRange_unique);
        ...
        for i in range(0, dateRange_unique.shape[0] ):
            #-----Leap Year Detection-----
            if np.mod(dateRange_unique[i,0],4) == 0: #leap year
                #-----Leap Year Skipped Detected - next will be 2100-----
                if( (np.mod(dateRange_unique[i,0],100) == 0) & (np.mod(dateRange_unique[i,0],400) != 0) ):
    ...
    #-----Option 2 returns the output in the [dayNum] only format-----
        ...
        dateRange_dayNum = np.int16(np.zeros( (dateRange[:,0].size ) ) ); #preallocate , numba happy
        tempMonth = np.int16(0); #prep to make numba happy
        dateRange_unique = np.unique(dateRange); #get the unique dates involved (helps if there are many repeats) 
        if( dateRange_unique.ndim == 1 ):
            dateRange_unique = nb_expand_dims(dateRange_unique);
