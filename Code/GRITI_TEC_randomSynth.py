...
def GRITI_TEC_randomSynth(deltavTECsize,TEC_lat,TEC_long,TEC_time, \
    ...
    settings, TEC_plotLimValu ,FLG_TEC_noise, FLG_plotStuff=0):
    ...
    if( FLG_TEC_noise == 1):
        ...
        deltavTECrando = np.random.default_rng().normal(loc=noise_background_mean,scale=noise_background_stdev,size=deltavTECsize); #fill in random noise into the vTEC (newer and faster!)
        ...
        if( FLG_plotStuff == 1 ):
            ...
            ax.set_xlabel('Longitude [arcdeg]',fontproperties=settings['plot']['font axis label FM']); #set the x axis label
            ax.set_ylabel('Latitude [arcdeg]',fontproperties=settings['plot']['font axis label FM']); #set the y axis label
        ...
        deltavTECrando += np.random.default_rng().normal(loc=noise_background_mean,scale=noise_background_stdev,size=(deltavTECsize,)); #delta_vTEC, calculate synthetic vTEC (newer and faster!)
    
        # from Code.GRITI_plot_area_scatter import GRITI_plot_area_scatter
        # GRITI_plot_area_scatter(deltavTECrando, TEC_time, TEC_lat, TEC_long, np.unique(TEC_time), np.unique(TEC_time), \
        #         settings['TEC'] ,settings['plot'], settings['paths'], settings['map'], \
        #         dates = None, boxArea = False, FLG_useAllTimes=True, FLG_fancyPlot = 0)
        # sys.crash()
    #END IF
