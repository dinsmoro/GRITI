...
def GRITI_TEC_avgPt_HP_timeMatch_POPL_plotWithISR_cutOut(avgPt_vTEC_timeMatch_HP_cutOut,avgPt_vTEC_timeMatch_time_cutOut,Zenith_time_cutOut,Zenith_height,Zenith_POPL_hp_cutOut,Zenith_POPL_hp_altAvgd_cutOut,MISA_time_cutOut,MISA_height,MISA_POPL_hp_cutOut,MISA_POPL_hp_altAvgd_cutOut,filter_cutoffPeriod,ISR_RTI_heightLimValues,ISR_POPL_plotLimValu,avgPt_coords,avgPt_ISRavgAlt,pointAltitude,time_cutout_range,dateRange,dateRange_dayNum,dateRange_dayNum_zeroHr,FONT_titleFM,FONT_axisTick,FONT_axisLabelFM,PLOT_lineWidth):
    ...
    line1, = ax[0].plot( (avgPt_vTEC_timeMatch_time_cutOut - dateRange_dayNum_zeroHr[1])*24 , avgPt_vTEC_timeMatch_HP_cutOut/np.mean(np.abs(avgPt_vTEC_timeMatch_HP_cutOut)) , color='xkcd:fire engine red' ,linewidth=PLOT_lineWidthRegularPlus, linestyle='-', marker='o'); # plot some time series
    line2, = ax[0].plot( (Zenith_time_cutOut - dateRange_dayNum_zeroHr[1])*24 , Zenith_POPL_hp_altAvgd_cutOut/np.mean(np.abs(Zenith_POPL_hp_altAvgd_cutOut)) , color='xkcd:electric blue' ,linewidth=PLOT_lineWidthRegularPlus, linestyle='--'); # plot some time series
    line3, = ax[0].plot( (MISA_time_cutOut - dateRange_dayNum_zeroHr[1])*24 , MISA_POPL_hp_altAvgd_cutOut/np.mean(np.abs(MISA_POPL_hp_altAvgd_cutOut)) , color='xkcd:vivid green' ,linewidth=PLOT_lineWidthRegularPlus, linestyle ='-'); # plot some time series
    
    ax[0].legend((line1, line2, line3), ('Zenith POPL High-passed AVG''d', 'MISA POPL High-passed AVG''d', 'delta-vTEC ISR Interval Matched AVG''d High-passed') ,loc='lower center', ncol=3, prop=FONT_axisLabelFM);
