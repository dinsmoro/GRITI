...
def GRITI_MagCAN_keo_area(data, dates, settings, keo_width, keo_range, temp_lats_up, temp_longs_up, temp_lat_list, temp_long_list):
    ...
    #==============UNPACK SETTINGS==============
    keo_angle = settings['MagCAN']['keo angle orig'];
    # keo_width = settings['MagCAN']['keo width orig'];
    ...
    for j in range(0,len(siteNames)):
        mN.append( ax.plot( data['MagCAN'][siteNames[j]]['long'], data['MagCAN'][siteNames[j]]['lat'], color=settings['plot']['color'][siteNamesIndex[j]], marker=settings['map']['marker type'], markersize=settings['map']['marker size'], linewidth=0, zorder=50, transform=settings['map']['projection'] ) ); #plot the sites
        sN.append( ax.text( data['MagCAN'][siteNames[j]]['long'], data['MagCAN'][siteNames[j]]['lat'], siteNames[j]+'\n  '+str(siteNamesIndex[j])+'', zorder=55, transform=settings['map']['projection']) ); #write name of site
    #END FOR j
    if( settings['MagCAN']['keo set stations'] == 1 ):
        ...
        for j in range(0,len(sitesRedacted)):
            mNR.append( ax.plot( data['MagCAN'][sitesRedacted[j]]['long'], data['MagCAN'][sitesRedacted[j]]['lat'], color=settings['plot']['color'][siteNamesIndex[j]], marker=settings['map']['marker type'], markersize=settings['map']['marker size'], linewidth=0, zorder=50, transform=settings['map']['projection'] ) ); #plot the sites
    ...
    string_Title = 'NR Canada MagCANnetic Observatories'+' Keo w/ Angle='+str(np.round(keo_angle,2))+'°, Keo Width='+ \
