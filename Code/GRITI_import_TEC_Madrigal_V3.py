...
def GRITI_import_TEC_Madrigal(dates, settings):
    ...
    version_filt = 3.1; #filtered algorithm version
    #1 10/8/2019 - initial algorithm
    ...
    for i in range(0,len(dateRange_dayNum_full[:,0])): #to download and unpack OR just unpack needed data
        ...
        if( (TEC_dataAvail[i] == 3) | (TEC_dataAvail[i] == 6) ): #3 means data was downloaded but needs to be converted (6 means data was downloaded but needs to be converted, but the current day is filtered and finished already)
            ...
            print("Converting {} to {}.\nAt {} data vectors and {} sec per vector, expect {} minutes for conversion to finish.\n".format(TEC_fileNameOrig,TEC_dataFileNameUnfilt[i],dataVectMax,current_timePerVect,str(np.round(current_timePerVect*dataVectMax/60,2)).rstrip('0').rstrip('.')));
        ...
        #                sys.stdout.flush();
                ...
                with h5py.File(settings_paths['data'] + '\\' + paths_TEC + '\\' + str(dateRange_full[i,0]) + '\\' + TEC_fileNameOrig, 'r') as TEC_fileOrig:
                    ...
                    unfilt_dict['sat'] = TEC_fileOrig[TEC_fileKeys[TEC_fileKeys_dataIndex]][:,'sat_type'].astype("int16"); #import satellite ID
                    dataVectCntr += 1; #increment
                    sys.stdout.write("\rData vector sat {}/{} & {} min\t\t\t\t".format(dataVectCntr,dataVectMax,np.round((time.time()-tic)/60,2))); #report
                    sys.stdout.flush();                    
                    ...
                    dataVectCntr += 1; #increment
                    sys.stdout.write("\rData vector time[temp] {}/{} & {} min\t\t\t\t".format(dataVectCntr,dataVectMax,np.round((time.time()-tic)/60,2))); #report
                    sys.stdout.flush();
                    time_temp *= 24; #hourify
                    unfilt_dict['hour'] = np.int16(time_temp); #get the hour
                    dataVectCntr += 1; #increment
                    sys.stdout.write("\rData vector hour calc {}/{} & {} min\t\t\t\t".format(dataVectCntr,dataVectMax,np.round((time.time()-tic)/60,2))); #report
                    sys.stdout.flush();
                    time_temp *= 60; #minutify
                    unfilt_dict['min'] = np.int16(time_temp - unfilt_dict['hour']*60); #get the minute
                    dataVectCntr += 1; #increment
                    sys.stdout.write("\rData vector min calc {}/{} & {} min\t\t\t\t".format(dataVectCntr,dataVectMax,np.round((time.time()-tic)/60,2))); #report
                    sys.stdout.flush();
                    time_temp *= 60; #secondify
                    unfilt_dict['sec'] = np.int16(time_temp - unfilt_dict['min']*60 - unfilt_dict['hour']*3600); #get the second
                    dataVectCntr += 1; #increment
                    sys.stdout.write("\rData vector sec calc {}/{} & {} min\t\t\t\t".format(dataVectCntr,dataVectMax,np.round((time.time()-tic)/60,2))); #report
                    sys.stdout.flush();
                    del time_temp
                    unfilt_dict['year'] = np.ones(unfilt_dict['sec'].size ,dtype=np.int16)*dateRange_dayNum_full[i,0];
                    dataVectCntr += 1; #increment
                    sys.stdout.write("\rData vector year calc {}/{} & {} min\t\t\t\t".format(dataVectCntr,dataVectMax,np.round((time.time()-tic)/60,2))); #report
                    sys.stdout.flush();
    ...
    #            TEC_timeUnique = TEC_timeUnique[ TEC_timeUniqueCount > TEC_dataLim ]; #remove the low data # stuff
                ...
                tic2 = time.time(); #get a timer for just calcs
                ...
                if( FLG_dataAggregation == 0 ): #with this off, just use what is directly there
                    ...
                    for j in range(0,unfilt_site_unique.size):
                        ...
                        for k in range(0,currentSatNSatType_unique.shape[0]):
                            ...
                            #END FOR l
                        #END FOR k
                        if( (np.mod(j+1,100) == 0) | (j == 0) | (j == unfilt_site_unique.size-1) ): #report only every 100 sites, makes it go quicker but still often enough to keep you up to the minute updated
                            toc = time.time() - tic2; #for time testing
                    ...
                    with h5py.File(settings_paths['data'] + '\\' + paths_TEC + '\\' + str(dateRange_full[i,0]) + '\\' + TEC_dataFileName[i], 'a') as TEC_file:
                        ...
                        if( coordType == 'geo' ):
                            ...
                            if( 'mlat' in keyzNew ):
                                ...
                                TEC_file.create_dataset('mlat', data=TEC_dataTemp_lat, chunks=TEC_dataTemp_lat.shape, compression="gzip"); #write that data
                                TEC_file.create_dataset('mlong', data=TEC_dataTemp_long, chunks=TEC_dataTemp_lat.shape, compression="gzip"); #write that data
