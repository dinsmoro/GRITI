...
def GRITI_keo_plot_sunCentered(keoData,timeUnique,plotLimValu,colorMap,plotLatRange,plotLongRange,latMillstone,longMillstone,
    ...
    #[(timeUnique - dateRange_dayNum_zeroHr[1]*86400)/3600 , avg_anyAngle_Range_Chunks_Long_Plot] ,
    ...
    xAxisTicks = np.arange( (np.round((np.min(timeUnique)-dateRange_dayNum_zeroHr[1]*86400)/3600) - np.mod(np.round((np.min(timeUnique)-dateRange_dayNum_zeroHr[1]*86400)/3600),2)) , \
            (np.round((np.max(timeUnique)-dateRange_dayNum_zeroHr[1]*86400)/3600) - np.mod(np.round((np.max(timeUnique)-dateRange_dayNum_zeroHr[1]*86400)/3600),2)) + 2 , \
    ...
    if( avg_anyAngle_Range_Chunks_Long_Plot_Name == 'Longitude' ): #if true, longitude
        if( (np.min(plotLongRange) <= midDegs[goalIndex]) & (np.max(plotLongRange) >= midDegs[goalIndex]) ): #only plot if it's in the long range specified
            ax.plot( np.linspace(np.min((timeUnique - dateRange_dayNum_zeroHr[1]*86400)/3600),np.max((timeUnique - dateRange_dayNum_zeroHr[1]*86400)/3600),10,endpoint=True) , #X time hr
