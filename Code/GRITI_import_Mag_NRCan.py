...
#sample rate second, data type variation is expected but others might work with massaging
expecting: 
 dates dictionary
 settings dictonary
...
def GRITI_import_Mag_NRCan(dates, settings, FLG_deleteRaw = 0, FLG_overwrite = 0):
    ...
    dataPath = os.path.join(settings['paths']['data'], 'Magnetometer'); #prep the data path
    dateRange_full = dates['date range full padded']; #get the full date range in daynum format out [increased to padded]
    dateRange_dayNum_full = dates['date range full padded dayNum']; #get the full date range in daynum format out [increased to padded]
    coordType = settings['map']['coord type']; #load in the coord type
    ...
    for i in range(0,dateRange_dayNum_full.shape[0]): #steps through each day
        ...
        dataPath_currFile = os.path.join(dataPath,str(dateRange_dayNum_full[i,0]),'NRCan_'+str(dateRange_dayNum_full[i,1])+'.h5'); #create the expected data path 
        #make sure folder exists
        if( os.path.isdir(os.path.join(dataPath, str(dateRange_dayNum_full[i,0]))) == False ):
            os.makedirs(os.path.join(dataPath, str(dateRange_dayNum_full[i,0]))); #make the path
        #END IF
    ...
    #                Mag_folders = list(filez.keys()); #lists the data folders in the HDF5 file
        ...
        if( (FLG_dataAlreadyProcessed != 1) | (FLG_overwrite == 1) ):
            ...
            import pandas
            from Code.subfun_strstr import strstr #this one is aware of \n's
            from Code.subfun_strstrNB import strstrNB #this one ignores \n's
            ...
            siteData = glob.glob( os.path.join(dataPath, str(dateRange_dayNum_full[i,0]), 'NRCan', '*'+str(dateRange_full[i,0])+str(dateRange_full[i,1]).zfill(2)+str(dateRange_full[i,2]).zfill(2)+'vsec.sec.gz') );
            # siteNames = np.array( np.zeros(len(siteData)), dtype='<U3' ); #preallocate
            siteNames = [None for j in range(0,len(siteData))]; #preallocate
            ...
            for j in range(0,len(siteNames)):
                ...
                #END IF
            #END FOR j
            if( (siteNamesFound.sum() != len(siteNames)) | (len(siteNames) == 0) ):
                ...
                sys.crash(); #crash the program
                #make sure folder exists
                if( os.path.isdir( os.path.join(dataPath, str(dateRange_dayNum_full[i,0]), 'NRCan') ) == False ):
                    os.makedirs( os.path.join(dataPath, str(dateRange_dayNum_full[i,0])), 'NRCan') ; #make the path
                #END IF
            #END IF
            
            #Prep mag data
            for j in range(len(siteNames)-1,-1,-1):
                #make sure only to record the available site names
                if( ~siteNamesFound[j] ):
                    siteNames.pop(j); #delete that entry, counts from backwards so can delete now ayy
                #END IF
            #END FOR j
            # siteNames = siteNames[siteNamesFound]; #make sure only to record the available site names [can't work b/c list for names, numpy ugh give special list mode for strings]
            ...
            for j in range(0,len(siteNames)):
                ...
                Mag_dataTemp_raw = pandas.read_csv( StringIO(Mag_dataTemp_raw), delim_whitespace=True, header=headerLine); #Read the data quick
            ...
            if( len(Mag_dataTemp['site names']) > 0 ): #prevents a file being made that just has a 1kb list of the site names
                with h5py.File(dataPath_currFile, 'w') as filez:
                    for j in range(0,len(Mag_dataTemp['site names'])):
                        ...
                        dataHolder = siteGroup.create_dataset('mag', Mag_dataTemp[Mag_dataTemp['site names'][j]]['mag'].shape, dtype='float32', compression='gzip', compression_opts=4, shuffle=True, fletcher32=True); #create dataset
                        dataHolder[...] = Mag_dataTemp[Mag_dataTemp['site names'][j]]['mag']; #write that data
                        dataHolder = siteGroup.create_dataset('magF', Mag_dataTemp[Mag_dataTemp['site names'][j]]['magF'].shape, dtype='float32', compression='gzip', compression_opts=4, shuffle=True, fletcher32=True); #create dataset
                        dataHolder[...] = Mag_dataTemp[Mag_dataTemp['site names'][j]]['magF']; #write that data
                        dataHolder = siteGroup.create_dataset('sec', Mag_dataTemp[Mag_dataTemp['site names'][j]]['sec'].shape, dtype='int32', compression='gzip', compression_opts=4, shuffle=True, fletcher32=True); #create dataset
                        dataHolder[...] = Mag_dataTemp[Mag_dataTemp['site names'][j]]['sec']; #write that data
                        dataHolder = siteGroup.create_dataset('secF', Mag_dataTemp[Mag_dataTemp['site names'][j]]['secF'].shape, dtype='int32', compression='gzip', compression_opts=4, shuffle=True, fletcher32=True); #create dataset
                        dataHolder[...] = Mag_dataTemp[Mag_dataTemp['site names'][j]]['secF']; #write that data
                        dataHolder = siteGroup.create_dataset('year', Mag_dataTemp[Mag_dataTemp['site names'][j]]['year'].shape, dtype='int16', compression='gzip', compression_opts=4, shuffle=True, fletcher32=True); #create dataset
                        dataHolder[...] = Mag_dataTemp[Mag_dataTemp['site names'][j]]['year']; #write that data
                        dataHolder = siteGroup.create_dataset('yearF', Mag_dataTemp[Mag_dataTemp['site names'][j]]['yearF'].shape, dtype='int16', compression='gzip', compression_opts=4, shuffle=True, fletcher32=True); #create dataset
                        dataHolder[...] = Mag_dataTemp[Mag_dataTemp['site names'][j]]['yearF']; #write that data
                        dataHolder = siteGroup.create_dataset('dayNum', Mag_dataTemp[Mag_dataTemp['site names'][j]]['dayNum'].shape, dtype='int16', compression='gzip', compression_opts=4, shuffle=True, fletcher32=True); #create dataset
                        dataHolder[...] = Mag_dataTemp[Mag_dataTemp['site names'][j]]['dayNum']; #write that data
                        dataHolder = siteGroup.create_dataset('dayNumF', Mag_dataTemp[Mag_dataTemp['site names'][j]]['dayNumF'].shape, dtype='int16', compression='gzip', compression_opts=4, shuffle=True, fletcher32=True); #create dataset
                    ...
                    #END FOR j
                    siteList = filez.create_dataset('siteNames', (len(Mag_dataTemp['site names']),), dtype='S3', compression='gzip', compression_opts=4, shuffle=True, fletcher32=True); #create dataset
                    siteList[...] = np.asarray(Mag_dataTemp['site names']).astype('S3'); #write that data
    ...
    keyz = Mag_data['site names']; #get the keyz
    for i in range(0,len(keyz)):
        for k in range(0,3):
            kk = Mag_data[keyz[i]]['mag'][:,k] >= 99999; #find outliers
            Mag_data[keyz[i]]['mag'][kk,k] = np.nan; #nan out the 99999's b/c they're no good
        #END FOR k
    #END FOR i
    
    #replace geo coords with mag coords if mag is on here - only a few calcs needed so its fast enough to not save it
    if( coordType == 'mag' ):
        import aacgmv2
        import datetime
        for j in range(0,len(siteNames)):
            time4mag_dayNmonth = subfun_dayNum_to_date( (np.median(Mag_data[siteNames[j]]['year']), np.median(Mag_data[siteNames[j]]['dayNum'])) ); #badabing badaboom
            time4mag_datetime = datetime.datetime(Mag_data[siteNames[j]]['year'][0], time4mag_dayNmonth[0,1], time4mag_dayNmonth[0,2], \
                                          hour = 12, minute = 0, second = 0); #date time object for aacgmv2 (choose middle of day since 1 number to represent and if 1 day then its middle of day yay)
            Mag_data[siteNames[j]]['lat'], Mag_data[siteNames[j]]['long'], _ = aacgmv2.convert_latlon(Mag_data[siteNames[j]]['lat'], Mag_data[siteNames[j]]['long'], 120., time4mag_datetime, method_code='G2A'); #converts from geographic to geomagnetic (AACGMv2) - vectorized _arr version has a memory leak atm
            #!note 120 chosen for altitude b/c of other works using 120 for magnetic field stuff!
            tryCntr = 1; #reset cntr
            while( (np.isnan(Mag_data[siteNames[j]]['lat']) | np.isnan(Mag_data[siteNames[j]]['long'])) & (tryCntr <= 20) ):
                #recalc at higher altitude to deal with error (idk how bad this is, but it allows the points to exist at least
                Mag_data[siteNames[j]]['lat'], Mag_data[siteNames[j]]['long'], _ = aacgmv2.convert_latlon(Mag_data[siteNames[j]]['lat'], Mag_data[siteNames[j]]['long'], 120.+200*tryCntr, time4mag_datetime, method_code='G2A'); #converts from geographic to geomagnetic (AACGMv2)
                tryCntr += 1 #increment
            #END IF
        #END FOR j
    #END IF
