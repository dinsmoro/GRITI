...
def GRITI_import_TEC_LISN(dateRange_dayNum_full,dateRange_dayNum_zeroHr, folder,plotLatRange,plotLongRange,web_LISN_creden_user,web_LISN_creden_pass, minElevation = 30, minimumTimeGap = 5, deltaTEC_compareValue = 3.5, TEC_dataRate = 30, filter_savGolPeriod = 1*60, order_savGol=1, filter_cutoffPeriod = 2, FLG_reqPaddedDays = 1, FLG_deleteOrig = 0, FLG_deleteUnfilt = 0, FLG_overwrite = 0, FLG_verboseReturn = 0 ,FLG_dataAggregation = 0 , TEC_dataAgg_timeAdditionLimit = 60, TEC_dataAgg_distToPts = 20, TEC_dataLimPercent = 0.05 , TEC_deltaLim = 0.5, TEC_timeTolerance = 0.1, TEC_maxAmpAllowed = 6 ):
    ...
    for i in range(0,len(dateRange_dayNum_full[:,0])): #to download and unpack OR just unpack needed data
        ...
        if( TEC_dataAvail[i] == 4 ): #4 means data was downloaded and converted but needs to be filtered (this is where 7 jumps off)
            ...
            windowLen_savGol = np.int64(np.round(filter_savGolPeriod/TEC_dataRate)); #window length, 60 minutes, converted to seconds, and divided by sample rate (plus 1 because odd is required) gets 121 for an hour window length (frame length)
                        ...
                        for k in range(0,currentSat_unique.size):
                            ...
                            for l in range(0,len(currentTimeSplits_loc)-1):
                                ...
                                (current_TEC_dataRate != 0) ):  #check to make sure some of the time range includes the day we want (otherwise we don't need to process it!) & == True for explicitness
                                    ...
                                    #---INTS---
                                    currentSat_singleSatLine = np.copy(currentSat[currentTimeSplits_loc[l]:currentTimeSplits_loc[l+1]]); #this will increase readability
                                    currentYear_singleSatLine = np.copy(currentYear[currentTimeSplits_loc[l]:currentTimeSplits_loc[l+1]]); #this will increase readability
                                    currentDayNum_singleSatLine = np.copy(currentDayNum[currentTimeSplits_loc[l]:currentTimeSplits_loc[l+1]]); #this will increase readability
                                    currentHour_singleSatLine = np.copy(currentHour[currentTimeSplits_loc[l]:currentTimeSplits_loc[l+1]]); #this will increase readability
                                    currentMin_singleSatLine = np.copy(currentMin[currentTimeSplits_loc[l]:currentTimeSplits_loc[l+1]]); #this will increase readability
                                    currentSec_singleSatLine = np.copy(currentSec[currentTimeSplits_loc[l]:currentTimeSplits_loc[l+1]]); #this will increase readability
                                    #---FLOATS---
                                    currentLat_singleSatLine = np.copy(currentLat[currentTimeSplits_loc[l]:currentTimeSplits_loc[l+1]]); #this will increase readability
                                    currentLong_singleSatLine = np.copy(currentLong[currentTimeSplits_loc[l]:currentTimeSplits_loc[l+1]]); #this will increase readability
                                    currentElv_singleSatLine = np.copy(currentElv[currentTimeSplits_loc[l]:currentTimeSplits_loc[l+1]]); #this will increase readability
                                    #---STRINGS---
                                    currentSite_singleSatLine = np.copy(currentSite[currentTimeSplits_loc[l]:currentTimeSplits_loc[l+1]]); #this will increase readability
                                    #---UNFILT DATA---
                                    ...
                                    currentsTECerror_singleSatLine = np.copy(currentsTECerror[currentTimeSplits_loc[l]:currentTimeSplits_loc[l+1]]); #this will increase readability
                                    ...
                                    if(np.any(currentSec_singleSatLine >= 60)):
                                        print('yo this is real bad');
                                        sys.crash();
                                    #END IF
                                    ...
                                    if( np.unique(currentSecTotal_singleSatLine).size == currentSecTotal_singleSatLine.size ):
                                        if( np.all(currentSecTotal_singleSatLine[:-1] <= currentSecTotal_singleSatLine[1:]) == False ):
                                            print('NOT SORTED AS MONOTOMICALLY INCREASING TIME CORRECTLY, CHECK IT OUT');
                                        ...
                                        if( (current_TEC_dataRate <= TEC_dataRate) & np.any(np.isin(currentSec_singleSatLine,TEC_dataRate_allowedStamps) == False) ): 
                                            ...
                                            if( np.any(track_Matches == False) ): #only run if there are still time stmaps that aren't at allowed times
                                                ...
                                                track_gaps = np.where( current_timeDiff > (TEC_dataRate*(1+TEC_timeTolerance)) )[0]; #catch time gaps that shouldn't be smashed together
    ...
    #                        currentElv = TEC_fileData_float[currentSite_loc,locFloatUnfilt_elev][currentSat_loc];
                            ...
                            currentTimeSplits_loc = np.append( np.insert( np.where(np.diff(currentDayNum)*1440 > minimumTimeGap)[0]+1 , 0, 0), currentDayNum.shape ); #get the locations where new non-contiguous data occurs - also tack on 0 and end#
                            ...
                            for l in range(0,len(currentTimeSplits_loc)-1):
                                currentDayNumF_singleSatLine = currentDayNum[currentTimeSplits_loc[l]:currentTimeSplits_loc[l+1]]; #this will increase readability
                                currentDayNumF_singleSatLine_min = (currentDayNumF_singleSatLine-np.round(np.mean(currentDayNumF_singleSatLine)))*144; #min/10, convert to minutes and /10 so it's more stable when fitting a curve
                                currentTEC_singleSatLine = currentTEC[currentTimeSplits_loc[l]:currentTimeSplits_loc[l+1]]; #this will increase readability
    ...
    #                            plt.show();
                                ...
                                if( (np.any(np.int16(currentDayNum[currentTimeSplits_loc[l]:currentTimeSplits_loc[l+1]]) == dateRange_dayNum_full[i,1]) == True) ):  #check to make sure some of the time range includes the day we want (otherwise we don't need to process it!) & == True for explicitness
                                    ...
                                    currentDataAgg_coefs = np.polynomial.polynomial.polyfit(currentDayNumF_singleSatLine_min, currentLat_singleSatLine, 3); #gets the coefficients that fit an x^2 polynomial - what a function to call am I right
    ...
    #                                    currentDataAgg_latGuess = np.hstack( (currentDataAgg_latGuessBefore, currentLat_singleSatLine , currentDataAgg_latGuessAfter) ); #degc, combine lat guess into one variable
                                    ...
                                    currentDataAgg_coefs = np.polynomial.polynomial.polyfit(currentDayNumF_singleSatLine_min, currentLong_singleSatLine, 3); #gets the coefficients that fit an x^2 polynomial - what a function to call am I right
    ...
    #                                plt.show();
                                ...
                                #make sure do work only on the day we want
                                #only get the data for times ranges thare are two times the filter period or longer
                                if( ( ((currentDayNum[currentTimeSplits_loc[l+1]-1]-currentDayNum[currentTimeSplits_loc[l]])*1440+currentDataAgg_number*TEC_dataRate/60) > filter_savGolPeriod*2) & #make sure the data time period is 2*filterPeriod
                                   ...
                                   (np.any(np.int16(currentDayNum[currentTimeSplits_loc[l]:currentTimeSplits_loc[l+1]]) == dateRange_dayNum_full[i,1]) == True) ):  #check to make sure some of the time range includes the day we want (otherwise we don't need to process it!) & == True for explicitness
                                    ...
                                    if( np.int16(currentDayNumF_singleSatLine[0]) != np.int16(currentDayNumF_singleSatLine[-1]) ): #if this is true, we are looking at different days' data being used
                                        ...
                                        #so the lines are continuous - but have a big jump where the different days are fit to different means(?) I'm not sure
                                        #but there's def a big jump we're gonna fix here
                                        currentDayNumF_singleSatLine_firstDayLocations = np.where( np.int16(currentDayNumF_singleSatLine[0]) == np.int16(currentDayNumF_singleSatLine) )[0]; #get index where 
                                        currentDayNumF_singleSatLine_secondDayLocations = np.where( np.int16(currentDayNumF_singleSatLine[-1]) == np.int16(currentDayNumF_singleSatLine) )[0]; #get index where 
                                        currentTEC_singleSatLine_diff = np.diff(currentTEC_singleSatLine); #get the deltas between the TEC values
                                        currentTEC_singleSatLine_firstDayLast = currentTEC_singleSatLine[currentDayNumF_singleSatLine_firstDayLocations[-1]]; #get the first day's last TEC value
                                        currentTEC_singleSatLine_secondDay = currentTEC_singleSatLine[currentDayNumF_singleSatLine_secondDayLocations]; #get the second day's TEC values
                                        if( currentDayNumF_singleSatLine_firstDayLocations.size > 1 ): #make sure currentDayNumF_singleSatLine_firstDayLocations[-2] exists
                                            currentTEC_singleSatLine_secondDay = currentTEC_singleSatLine_secondDay*( currentTEC_singleSatLine_firstDayLast + currentTEC_singleSatLine_diff[currentDayNumF_singleSatLine_firstDayLocations[-2]] )/currentTEC_singleSatLine_secondDay[0]; #scale it so the first data pt of the second day is now the last data pt of the first day plus the delta between the first day's last and second to last pt
                                            #sorry about these confusing explanations but it def makes sense to me rightnow
                                        else: #if it doesn't, time to use currentDayNumF_singleSatLine_secondDayLocations[0] instead of currentDayNumF_singleSatLine_firstDayLocations[-2] (so projecting from reverse instead of forward)
                                            currentTEC_singleSatLine_secondDay = currentTEC_singleSatLine_secondDay*( currentTEC_singleSatLine_firstDayLast + currentTEC_singleSatLine_diff[currentDayNumF_singleSatLine_secondDayLocations[0]] )/currentTEC_singleSatLine_secondDay[0]; #scale it so the first data pt of the second day is now the last data pt of the first day plus the delta between the first day's last and second to last pt
                                            #sorry about these confusing explanations but it def makes sense to me rightnow
                                        #END IF
                                        currentTEC_singleSatLine[currentDayNumF_singleSatLine_secondDayLocations] = currentTEC_singleSatLine_secondDay; #put em in
                                    #END IF
    ...
    #                                plt.figure();
    #                                plt.scatter( currentDayNumF_singleSatLine , currentTEC_singleSatLine , 20 , "r" );
    ...
    #                                    plt.figure();
    #                                    plt.scatter( currentDataAgg_dayNumSSLExtended , currentDataAgg_TEC , 20 , "r" );
    #                                    plt.scatter( currentDayNumF_singleSatLine , currentTEC_singleSatLine , 20 );
    ...
    #                                if(  currentDataAgg_number > 0 ):
    #                                    plt.figure();
    #                                    plt.scatter( currentDayNumF_singleSatLine , current_deltaTEC );
    ...
    #                                #END IF
        ...
        #                            currentTEC_singleSatLine = currentTEC[currentTimeSplits_loc[l]:currentTimeSplits_loc[l+1]]; #this will increase readability
                                    current_deltaTEC = np.ones(currentTimeSplits_loc[l+1]-currentTimeSplits_loc[l],dtype=np.float32)*np.nan; #no filter, just filler (NaN to easy identify)
                                #END IF
                                
                                #no matter what, write it in!
        #                        TEC_fileData_float[currentSite_loc,locFloatUnfilt_dTEC][currentSat_loc[currentTimeSplits_loc[l]:currentTimeSplits_loc[l+1]]]  = current_deltaTEC; #save that deltaTEC into wherever it should go - assigning with double indexing DOES NOT WORK
    #                                TEC_fileData_float[currentSite_loc[currentSat_loc[currentTimeSplits_loc[l]:currentTimeSplits_loc[l+1]]],locFloatUnfilt_dTEC] = current_deltaTEC; #save that deltaTEC into wherever it should go - getting around double indexing
                                # TEC_fileData_dTEC[currentSite_loc[currentSat_loc[currentTimeSplits_loc[l]:currentTimeSplits_loc[l+1]]]] = current_deltaTEC; #save that deltaTEC into wherever it should go - getting around double indexing
                                #WRONG BELOW
        #                        TEC_fileData_float[currentSite_loc[currentTimeSplits_loc[l]:currentTimeSplits_loc[l+1]],locFloatUnfilt_dTEC] = current_deltaTEC; #save that deltaTEC into wherever it should go - double illegal indexing didn't work but moving it in was the same indexing so safe
                            #END FOR l
                        #END FOR k
                    #ENF FOR j
    ...
    TEC_data['site'] = TEC_string; #it's not a vector
    del TEC_string #clear mem
    gc.collect(); #force the mem clear
    #get the attributes
