...
"""
import numpy as np
from Code.GRITI_TEC_avgPt_timeMatchACCELERATOR import GRITI_TEC_avgPt_timeMatchACCELERATOR
...
#    jk = np.where( np.isnan(avgPT_vTEC_timeMatch) == 1)[0]; #find NaNs existing
    ...
    avgPt_vTEC_timeMatch_HP = subfun_highpass( avgPt_vTEC_timeMatch_time , avgPt_vTEC_timeMatch,filter_cutoffPeriod=filter_cutoffPeriod); #highpass that data to boot
