...
def subfun_highpass(timeData, data, filter_cutoffPeriod = 2*3600, filter_order = 42, windowType = 'hann', axisToUse = 1, reduceWindow = 0):
...
#    n=42; # order of the Hamming window used in the custom function (uses 43 in reality)
    ...
    b = signal.firwin(filter_order+1, wp, window=windowType, pass_zero=False); #just high-pass (2 hr and lower OK)
    a = 1; #for FIR a is 1
    ...
    if(data.ndim == 1):
        ...
        data_hp = signal.filtfilt(b,a,data,axis=axisToUse,padtype = 'odd', padlen=padlen); #Appies the filter
        #extra bits from https://dsp.stackexchange.com/questions/11466/differences-between-python-and-matlab-filtfilt-function
    #END IF
