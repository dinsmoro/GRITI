...
def GRITI_ISR_Haystack_plot_ionVel_HP_cutOut(Zenith_time_cutOut,Zenith_height,Zenith_vel_hp_cutOut,MISA_time_cutOut,MISA_height,MISA_vel_hp_cutOut,ISR_RTI_heightLimValues,ISR_ionVel_plotLimValu,ISR_cutoffPeriod,time_cutout_range,dateRange,dateRange_dayNum,dateRange_dayNum_zeroHr,FONT_titleFM,FONT_axisTick,FONT_axisLabelFM):
    ...
    pltHelprX, pltHelprY = np.meshgrid( (Zenith_time_cutOut - dateRange_dayNum_zeroHr[1])*24, \
                Zenith_height);
    im = ax[0].pcolormesh(pltHelprX , pltHelprY , Zenith_vel_hp_cutOut , vmin=np.min(ISR_ionVel_plotLimValu) , vmax=np.max(ISR_ionVel_plotLimValu) , cmap='gray' , shading='gouraud'); # pseudocolor plot "stretched" to the grid
    cbar = fig.colorbar(im, cax=cax, orientation='vertical'); #create a colorbar using the prev. defined cax
    cbar.mappable.set_clim(vmin=np.min(ISR_ionVel_plotLimValu), vmax=np.max(ISR_ionVel_plotLimValu));
    ...
    string_Title = 'Zenith LOS Ion Velocity with High-pass Filter '+str(ISR_cutoffPeriod)+' hr Cutoff - Hours '+str(np.min(time_cutout_range))+' to '+str(np.max(time_cutout_range)); #create mecha title
    ...
    ax[0].set_xlim( ( np.min(time_cutout_range) , np.max(time_cutout_range) ) ); #set x axis limits
    ...
    pltHelprX, pltHelprY = np.meshgrid( (MISA_time_cutOut - dateRange_dayNum_zeroHr[1])*24, \
                MISA_height);
    im = ax[1].pcolormesh(pltHelprX , pltHelprY , MISA_vel_hp_cutOut , vmin=np.min(ISR_ionVel_plotLimValu) , vmax=np.max(ISR_ionVel_plotLimValu) , cmap='gray', shading='gouraud'); # pseudocolor plot "stretched" to the grid
    cbar = fig.colorbar(im, cax=cax, orientation='vertical'); #create a colorbar using the prev. defined cax
    cbar.mappable.set_clim(vmin=np.min(ISR_ionVel_plotLimValu), vmax=np.max(ISR_ionVel_plotLimValu));
    #cbar.ax.set_yticklabels(np.round(np.linspace(-ISR_POPL_plotLimValu,ISR_POPL_plotLimValu,5), len(str(ISR_POPL_plotLimValu).split('.')[1])+1 )); #create useful tick marks
    cbar.set_label("Ion Velocity [$m/s$]"); #tabel the colorbar
#    cax.yaxis.set_major_formatter(FormatStrFormatter('%.2f')); #force a rounded format
    ...
    string_Title = 'MISA LOS Ion Velocity with High-pass Filter '+str(ISR_cutoffPeriod)+' hr Cutoff - Hours '+str(np.min(time_cutout_range))+' to '+str(np.max(time_cutout_range)); #create mecha title
...
#    ax[1].set_xlim( ((np.min(MISA_time_cutOut)-dateRange_dayNum_zeroHr[1])*24 , (np.max(MISA_time_cutOut)-dateRange_dayNum_zeroHr[1])*24) ); #set x axis limits
    ax[1].set_xlim( ( np.min(time_cutout_range) , np.max(time_cutout_range) ) ); #set x axis limits
