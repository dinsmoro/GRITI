...
def GRITI_spectral_6minAnalysisPlot(dataz, timez, timezUnit, dataRate, dataRateUnit, plotTimeUnit, \
    ...
    if( FLG_fancyPlot >= 1 ):
        if( titleOverride != None ):
            ...
            #END FOR j
        #END IF
        print('MAKING FANCY PLOT: spectra_6min_'+fileNamer+' IN fancyPlot FOLDER'); #report since you won't see anything
    #END IF
    ...
    for j in range(0,datazOverallLen):
        ...
        for i in range(0,datazLen):
            ...
            #--- PREP - time Match to 6 min (360 sec) ---
            if( np.abs(dataRate[j][i]-360) > 0.05 ):
                sixMin_timeUnique_sec = np.arange(timez[j][i][0],timez[j][i][-1]+dataRate[j][i],360); #sec, arange time stamps
                ...
                dataz[j][i], timez[j][i] = subfun_timeMatch(dataz[j][i], timez[j][i], sixMin_timeUnique_sec, timeMatch_delta=360, FLG_removeNaNs=1, FLG_useSum=0);
                dataRate[j][i] = 360; #record the new data rate
            #END IF
    ...
    #        if( OMNI_data_scarggf < np.max[j](OMNI_data_scargPower[OMNI_data_scargPeriod<=plot_Period_Lim])+0.1*np.max[j](OMNI_data_scargPower[OMNI_data_scargPeriod<=plot_Period_Lim]) ):
    #            ax[j].set_ylim( (0, np.max[j](OMNI_data_scargPower[OMNI_data_scargPeriod<=plot_Period_Lim])+0.1*np.max[j](OMNI_data_scargPower[OMNI_data_scargPeriod<=plot_Period_Lim]) ) ); #set x axis limits
    #        else:
    ...
    if( FLG_fancyPlot != 0 ):
        fig.savefig(os.path.join(settings_paths['fancyPlots'],'spectra_6min_'+fileNamer+'.png')); #save the figure
