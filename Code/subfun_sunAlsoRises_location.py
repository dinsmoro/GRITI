...
#dateRange_full should be the individual days, no repeats, involved in the format:
#   np.array([
#   [2013, 126],
#   [2013, 127],
#   [2013, 128]
#   ])
#timeIndexes are time indexes PER day, they can be day+sec times OR repeating sets from 0 to 86400 (0, 43200, 86400, 0 43200, 86400, 0, 43200, 86400) etc..
#or use dataRate to dynamically generate for the day indexes above
def sunAlsoRises_location(dateRange_full,timeIndexes=None,dataRate=None,lat=None,long=None,timeZone=None,timeZoneCredentials=None):
    
    # make sure we're working with YR/M/D
    ...
    #END IF
    if( np.all(timeIndexes != None) ):
        #If timeIndexes were provided and do not cover the full dateRange_full range, clip dateRange_full to match so things work later
        if( np.unique(timeIndexes//86400).size != dateRange_full.shape[0] ):
            dateRange_full = subfun_time_to_dateRange(timeIndexes, dates_zeroHr = dateRange_full[dateRange_full.shape[0]//2,:], FLG_timesWRTzeroHr = False, options = 1); #convert
        #END IF
    #END IF
    
    if( (np.any((lat == None) | (long == None)) == False) & (timeZoneCredentials != None) ):
        ...
        if dayNite_timeZoneID is None:
            ...
            url = 'http://api.geonames.org/timezone?lat='+str(lat)+'&lng='+str(long)+'&username='+timeZoneCredentials; #create link for lat/long
    ...
    if( np.any(timeIndexes != None) ):
        if( type(timeIndexes) is list ):
            ...
            FLG_secs = False;
            for j in range(0, len(localTime)):
                if( localTime[j] > 24 ):
                    FLG_secs = True; #assume seconds if greater than 24
                #END IF
            #END FOR j
            if( FLG_secs == True ):
                for j in range(0, len(localTime)):
                    localTime[j] = localTime[j]/3600; #convert to hrs as needed
                #END FOR j
            #END IF
            FLG_outType = 'list';
        elif( np.isscalar(timeIndexes) ):
            #it's one time index
            if( timeIndexes > 86400 ):
                if( dateRange_full.shape[0] > 1 ):
                    print('ERROR in sunAlsoRises_location: ONE timeIndexes provided ('+str(timeIndexes)+') that is greater than 86400 (on a specific day) but multiple dateRange_full provided as well ('+str(dateRange_full)+'). Code isn\'t good enough for that. Returning an error.'); #report error
                    import sys
                    sys.crash() #better
                    return 'Error in sunAlsoRises_location' #very helpful
                else:
                    localTime = np.tile(np.mod(timeIndexes,86400),dateRange_full.shape[0])/3600; #convert to per day time, assume sec, convert into an array, assume sec -> hr conversion
                #END IF
            else:
                localTime = np.tile(timeIndexes,dateRange_full.shape[0])/3600; #make a local time list that covers all the dates input, assume sec -> hr conversion
            #END IF
    ...
    sunSubSolar_loc = {'lat':[None for j in range(0,dateRange_full.shape[0])],
                   'long':[None for j in range(0,dateRange_full.shape[0])],
                   }; #prep a dict
    ...
    #END FOR i
    if( FLG_outType == 'flat' ):
        sunSubSolar_loc['lat'] = np.concatenate(sunSubSolar_loc['lat']).ravel(); #flatten into an array
        sunSubSolar_loc['long'] = np.concatenate(sunSubSolar_loc['long']).ravel(); #flatten into an array
    #END IF
