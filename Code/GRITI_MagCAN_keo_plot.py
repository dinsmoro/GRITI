...
def GRITI_MagCAN_keo_plot(data, dates, settings):
    ...
    pltHelprX, pltHelprY = np.meshgrid( data['MagCAN']['keo time hr'], \
                settings['MagCAN']['keo plot range chunks']); 
        #,vmin=np.min(TEC_plotLimValu), vmax=np.max(TEC_plotLimValu)
    im = ax.pcolormesh(pltHelprX, pltHelprY,  data['MagCAN']['keo'].T ,cmap=settings['MagCAN']['keo colorbar']); # pseudocolor plot "stretched" to the grid
    cbar = fig.colorbar(im, cax=cax, orientation='vertical'); #create a colorbar using the prev. defined cax
    if( settings['MagCAN']['keo plot lim value'] != None ): 
        cax.yaxis.set_ticks(np.linspace(np.min(settings['MagCAN']['keo plot lim value']),np.max(settings['MagCAN']['keo plot lim value']),5)); #create useful tick marks
    #END IF
    # cax.yaxis.set_major_formatter(FormatStrFormatter('%.2f')); #force a rounded format
    cbar.set_label(settings['MagCAN']['keo plot name']); #tabel the colorbar
    ...
    #END FOR tick
    if( settings['MagCAN']['keo plot lim value'] != None ): 
        #cbar.set_clim(vmin=np.min(settings['MagCAN']['keo plot lim value']), vmax=np.max(settings['MagCAN']['keo plot lim value'])); #they changed how the code works, this doesn't work anymore
        cbar.mappable.set_clim(vmin=np.min(settings['MagCAN']['keo plot lim value']), vmax=np.max(settings['MagCAN']['keo plot lim value'])); #now it's this
    #END IF
    ...
    #        ' arcdeg, Line Shows '+settings['MagCAN']['keo plot name']+' of Millstone Hill Zenith Beam'; #create mecha title
    if( settings['MagCAN']['delta method'] == 'mean' ):
        string_titleTemp = '0 Mean MagCAN'; #set the specific word to describe the data
    elif( settings['MagCAN']['delta method'] == 'savgol' ):
        string_titleTemp = 'Delta-MagCAN'; #set the specific word to describe the data
    else: #otherwise do nothing
        string_titleTemp = 'MagCAN'; #set the specific word to describe the data
    #END IF
    if( settings['MagCAN']['keo normalize'] == 1 ):
        string_titleTemp = string_titleTemp+' Norm\'d'; #set the specific word to describe the data
    #END IF
    string_Title = string_titleTemp+' Keo Averaged on Angle of '+str(settings['MagCAN']['keo angle orig'])+' deg and Width of '+ \
        str(np.round(settings['MagCAN']['keo width'],2))+' arcdeg'; #create mecha title
    ...
    ax.set_ylabel(settings['MagCAN']['keo plot range name']+' [arcdeg]',fontproperties=settings['plot']['font axis label FM']); #set the y axis label
    
    xAxisTicks = np.arange( (np.round(np.min(data['MagCAN']['keo time hr'])) - np.mod(np.round(np.min(data['MagCAN']['keo time hr'])),2)) , \
            (np.round(np.max(data['MagCAN']['keo time hr'])) - np.mod(np.round(np.max(data['MagCAN']['keo time hr'])),2)) + 4 , \
            4); #sets the start hr, stop hr, and the step size between (in this case, 4 hr)
    ax.set_xticks(xAxisTicks); #set x axis ticks
    
    kep_range_chunks_long_plot_autoTick = (np.ceil(np.max(settings['MagCAN']['keo plot range chunks'])) - np.floor(np.min(settings['MagCAN']['keo plot range chunks'])))/13; #tries to split the latitude range into 13 parts (based off of 180/15+1)
    if( kep_range_chunks_long_plot_autoTick > 25 ):
        ...
        if(settings['MagCAN']['keo plot range name'] == 'Latitude'): #if Y axis is latitude, use latitude
            kep_range_chunks_long_plot_autoTick = (np.max(settings['MagCAN']['lat range']) - np.min(settings['MagCAN']['lat range']))/13; #just goes for it if it's a super tiny range
        elif(settings['MagCAN']['keo plot range name'] == 'Longitude'): #if Y axis is longitude, use longitude
            kep_range_chunks_long_plot_autoTick = (np.max(settings['MagCAN']['long range']) - np.min(settings['MagCAN']['long range']))/13; #just goes for it if it's a super tiny range
        #END IF
    #END IF
    yAxisTicks = np.round(np.arange( np.floor(np.min(settings['MagCAN']['keo plot range chunks'])),np.ceil(np.max(settings['MagCAN']['keo plot range chunks'])),kep_range_chunks_long_plot_autoTick ),2); #creates y ticks autoMagCANically
    ax.set_yticks(yAxisTicks); #set x axis ticks
    
    # #Now drawing line of interest
    # if( settings['MagCAN']['keo plot range name'] == 'Longitude' ): #if true, longitude
