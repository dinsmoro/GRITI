...
def GRITI_TEC_keo_plot_area(TEC_time,TEC_dTEC,\
    ...
    if( avg_anyAngle_polarMode == 0):
        ...
        for j in np.arange(0,360,30): #longitude labels
            ...
            #END IF
            ax.text(x,y,textNice(angle)+'\N{DEGREE SIGN}',transform=ax.transAxes,horizontalalignment='center',verticalalignment='center',fontproperties=FONT_axisTickFM)
        #END FOR j
        ...
        for j in range(0,latPts.size): #latitude labels
            ...
            ax.text(x,y,textNice(latPts[j])+'\N{DEGREE SIGN}',transform=ax.transAxes,horizontalalignment='center',verticalalignment='center',fontproperties=FONT_axisTickFM)
            #ax.text(x,y,textNice(latPts[j])+'\N{DEGREE SIGN}',horizontalalignment='center',verticalalignment='center',fontproperties=FONT_axisTickFM)
        #END FOR j
    ...
    if( np.any(np.isinf(TEC_plotLimValu)) == False ):
        ...
        cax.yaxis.label.set_font_properties(FONT_axisLabelFM);
    #END IF
    string_Title = avg_anyAngle_dataType+' Avging w/ Angle='+textNice(np.round(avg_anyAngle,2))+' deg, Avg Width='+ \
        textNice(np.round(avg_anyAngle_Width,2))+' arcdeg, Step #='+textNice(avg_anyAngle_N)+ \
        ', Step Width='+ \
        textNice(np.round(np.sqrt( (temp_Longs_up[0,0] - temp_Longs_up[0,1])**2 + (temp_Lats_up[0,0] - temp_Lats_up[0,1])**2 ),2))+ \
        ' arcdeg | '+textNice(dateRange_dayNum_zeroHr[1])+', '+textNice(dateRange_dayNum_zeroHr[0])+' at '+textNice(timeHr)+':'+textNice(timeMin)+':'+textNice(timeSec); #create mecha title
