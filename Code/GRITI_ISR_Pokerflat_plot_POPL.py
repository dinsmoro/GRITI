...
def GRITI_ISR_Pokerflat_plot_POPL(PFISR_time,PFISR_height,PFISR_POPL,PFISR_el,PFISR_az,ISR_cutoffPeriod,ISR_RTI_heightLimValues,dateRange,dateRange_dayNum,dateRange_dayNum_zeroHr,FONT_titleFM,FONT_axisTick,FONT_axisLabelFM):
    ...
    for j in range(0,len(PFISR_time)):
        ...
        pltHelprX, pltHelprY = np.meshgrid( (PFISR_time[j] - dateRange_dayNum_zeroHr[1]*86400)/3600, \
                    PFISR_height[j]);
        im = ax.pcolormesh(pltHelprX , pltHelprY , PFISR_POPL[j].T , cmap='gray' , shading='gouraud'); # pseudocolor plot "stretched" to the grid
        ...
        string_Title = 'PFISR POPL - No Filters | El = '+str(np.median(PFISR_el[j]))+' & Az = '+str(np.median(PFISR_az[j])); #create mecha title
