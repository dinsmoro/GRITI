...
def GRITI_ISR_Pokerflat_plot_POPL_HP(PFISR_time,PFISR_height,PFISR_POPL_hp,PFISR_el,PFISR_az,ISR_cutoffPeriod,ISR_RTI_heightLimValues,ISR_POPL_plotLimValu,dateRange,dateRange_dayNum,dateRange_dayNum_zeroHr,FONT_titleFM,FONT_axisTick,FONT_axisLabelFM):
    ...
    for j in range(0,len(PFISR_time)):
        ...
        pltHelprX, pltHelprY = np.meshgrid( (PFISR_time[j] - dateRange_dayNum_zeroHr[1]*86400)/3600, \
                    PFISR_height[j]);
        im = ax.pcolormesh(pltHelprX , pltHelprY , PFISR_POPL_hp[j].T , vmin=np.min(ISR_POPL_plotLimValu) , vmax=np.max(ISR_POPL_plotLimValu) , cmap='gray' , shading='gouraud'); # pseudocolor plot "stretched" to the grid
        ...
        string_Title = 'PFISR POPL with High-pass Filter '+textNice(ISR_cutoffPeriod/3600)+' hr Cutoff | El = '+str(np.median(PFISR_el[j]))+' & Az = '+str(np.median(PFISR_az[j])); #create mecha title
...
#        ax.set_ylabel('Height (km)',fontproperties=FONT_axisLabelFM); #set the y axis label
        ...
        xAxisTicks = np.arange( (np.round((np.min(PFISR_time[j])-dateRange_dayNum_zeroHr[1]*86400)/3600) - np.mod(np.round((np.min(PFISR_time[j])-dateRange_dayNum_zeroHr[1]*86400)/3600),2)) , \
                (np.round((np.max(PFISR_time[j])-dateRange_dayNum_zeroHr[1]*86400)/3600) - np.mod(np.round((np.max(PFISR_time[j])-dateRange_dayNum_zeroHr[1]*86400)/3600),2)) + 2 , \
                2); #sets the start hr, stop hr, and the step size between (in this case, 2 hr)
        ax.set_xticks(xAxisTicks); #set x axis ticks
        ax.set_xlim( ((np.min(PFISR_time[j])-dateRange_dayNum_zeroHr[1]*86400)/3600 , (np.max(PFISR_time[j])-dateRange_dayNum_zeroHr[1]*86400)/3600) ); #set x axis limits
