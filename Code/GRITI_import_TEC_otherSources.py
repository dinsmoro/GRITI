...
def GRITI_import_TEC_otherSources(dateRange_dayNum_full,dateRange_dayNum_zeroHr,settings_paths,plotLatRange,plotLongRange,TEC_dataRate=30,TEC_minimumTimeGap=5,FLG_dataMix=0,FLG_dataPreference=0,TEC_dataLimPercent = 0.05,TEC_timeTolerance=0.1):
...
#                TEC_str = np.concatenate( (TEC_str ,  np.repeat(np.array( '~~~~' ,dtype='S4'),TEC_dataTemp.shape[0])) , axis=0); #mass add ~~~~'s
    ...
    TEC_data['version'] = np.array((1.0,)); #set version to 1 - will not change
    TEC_data['pierceAlt'] = np.array((325,)); #set version to 1 - will not change
