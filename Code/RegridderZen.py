"""
This function regrids an array to a new array size through matrix math, has support for large array speedups with sparce matricies
inputs:
...
def RegridderZen(inArray, outSize):
    ...
    for i in range(0,yRangeAdj.size): # Run through all of the y ranges and adjust as needed
        if( np.isclose(yRangeAdj[i], int(yRangeAdj[i])) ): # Catch true integers
