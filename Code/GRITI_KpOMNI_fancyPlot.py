...
def GRITI_KpOMNI_fancyPlot(Kp_data,Kp_time, OMNI_data,OMNI_timeUnique, \
    ...
    xAxisTicksOMNI, _ = GRITI_plotHelper_axisizerTime(OMNI_timeUnique_hr); #automagic time ticks here
    ...
    gridr = gridspec.GridSpec(nrows=len(OMNI_plotSet_name)*2+1, ncols=1, figure=fig);
    gridr.update(hspace=0.05); # set the spacing between axes. 
    fig.add_subplot(gridr[0:1]);
    for i in range(1,len(OMNI_plotSet_name)*2+1,2):
        fig.add_subplot(gridr[i:i+2]); #dynamically add axes
    #END FOR i
...
#    ax.set_yticks(yAxisTicks); #set x axis ticks
    ...
    for i in range(0,len(OMNI_plotSet_name)):
        ...
        if( (OMNI_plotSet_name[i] == 'IMF clock angle') | (OMNI_plotSet_name[i] == 'Bz GSM') | (OMNI_plotSet_name[i] == 'Vsw') | (OMNI_plotSet_name[i] == 'Psw') | (OMNI_plotSet_name[i] == 'SYM/H') ):
            ...
            kk2[kk] = np.nan;
            ax[i+1].plot( OMNI_timeUnique_hr, kk2 , linewidth=PLOT_lineWidthSmol, color='xkcd:brick red', antialiased=True); #plot
        #END IF
