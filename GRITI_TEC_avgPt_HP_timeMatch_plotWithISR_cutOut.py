...
def GRITI_TEC_avgPt_HP_timeMatch_plotWithISR_cutOut(avgPt_vTEC_timeMatch_HP_cutOut,avgPt_vTEC_timeMatch_time_cutOut,Zenith_time_cutOut,Zenith_height,Zenith_SNR_hp_cutOut,Zenith_SNR_hp_altAvgd_cutOut,MISA_time_cutOut,MISA_height,MISA_SNR_hp_cutOut,MISA_SNR_hp_altAvgd_cutOut,filter_cutoffPeriod,ISR_RTI_heightLimValues,ISR_plotLimValu,avgPt_coords,avgPt_ISRavgAlt,pointAltitude,time_cutout_range,dateRange,dateRange_dayNum,dateRange_dayNum_zeroHr,FONT_titleFM,FONT_axisTick,FONT_axisLabelFM,PLOT_lineWidth):
    ...
    line1, = ax[0].plot( (avgPt_vTEC_timeMatch_time_cutOut - dateRange_dayNum_zeroHr[1])*24 , avgPt_vTEC_timeMatch_HP_cutOut , color='xkcd:fire engine red' ,linewidth=PLOT_lineWidth, linestyle='-', marker='o'); # plot some time series
    line2, = ax[0].plot( (Zenith_time_cutOut - dateRange_dayNum_zeroHr[1])*24 , Zenith_SNR_hp_altAvgd_cutOut , color='xkcd:electric blue' ,linewidth=PLOT_lineWidth, linestyle='--'); # plot some time series
    line3, = ax[0].plot( (MISA_time_cutOut - dateRange_dayNum_zeroHr[1])*24 , MISA_SNR_hp_altAvgd_cutOut , color='xkcd:vivid green' ,linewidth=PLOT_lineWidth, linestyle ='-'); # plot some time series
    
    ax[0].legend((line1, line2, line3), ('Zenith SNR High-passed AVG''d', 'MISA SNR High-passed AVG''d', 'delta-vTEC ISR Interval Matched AVG''d High-passed') ,loc='lower center', ncol=3);
    string_Title = 'delta-vTEC ISR Interval Matched AVG''d High-passed AND Zenith/MISA SNR HP AVG''d +/-'+str(avgPt_ISRavgAlt)+' km around '+str(pointAltitude)+' km at '+str(np.round(avgPt_coords[0],2))+' deg lat/'+str(np.round(avgPt_coords[1],2))+'deg long'; #create mecha title
...
#    ax[0].set_xlabel(subfun_monthNum_to_word(dateRange[0,1])[0]+" "+str(dateRange[0,2])+" (Day "+str(dateRange_dayNum[0,1])+"), "+str(dateRange[0,0])+" to "+subfun_monthNum_to_word(dateRange[1,1])[0]+" "+str(dateRange[1,2])+" (Day "+str(dateRange_dayNum[0,1])+"), "+str(dateRange[1,0]),fontproperties=FONT_axisLabelFM); #set the x axis label
    ax[0].set_ylabel('Amplitude [TECU/Unitless]',fontproperties=FONT_axisLabelFM); #set the y axis label
...
#    cax = divider.append_axes('right', size='2.0%', pad=0.35); #make a color bar axis
    pltHelprX, pltHelprY = np.meshgrid( (Zenith_time_cutOut - dateRange_dayNum_zeroHr[1])*24, \
                Zenith_height);
    im = ax[1].pcolormesh(pltHelprX , pltHelprY , Zenith_SNR_hp_cutOut , vmin=-ISR_plotLimValu , vmax=ISR_plotLimValu , cmap='gray', shading='gouraud'); # pseudocolor plot "stretched" to the grid
#    cbar = fig.colorbar(im, cax=cax, orientation='vertical'); #create a colorbar using the prev. defined cax
#    cbar.set_clim(vmin=-ISR_plotLimValu, vmax=ISR_plotLimValu);
...
#    #cbar.set_clim( (-ISR_plotLimValu,ISR_plotLimValu) ); #create useful tick marks
    ...
    string_Title = 'Zenith SNR with High-pass Filter '+str(filter_cutoffPeriod)+' hr Cutoff - Hours '+str(np.min(time_cutout_range))+' to '+str(np.max(time_cutout_range)); #create mecha title
...
#    cax = divider.append_axes('right', size='2.0%', pad=0.35); #make a color bar axis
    pltHelprX, pltHelprY = np.meshgrid( (MISA_time_cutOut - dateRange_dayNum_zeroHr[1])*24, \
                MISA_height);
    im2 = ax[2].pcolormesh(pltHelprX , pltHelprY , MISA_SNR_hp_cutOut , vmin=-ISR_plotLimValu , vmax=ISR_plotLimValu , cmap='gray', shading='gouraud'); # pseudocolor plot "stretched" to the grid
#    cbar = fig.colorbar(im, cax=cax, orientation='vertical'); #create a colorbar using the prev. defined cax
#    cbar.set_clim(vmin=-ISR_plotLimValu, vmax=ISR_plotLimValu);
#    #cbar.ax.set_yticklabels(np.round(np.linspace(-ISR_plotLimValu,ISR_plotLimValu,5), len(str(ISR_plotLimValu).split('.')[1])+1 )); #create useful tick marks
#    #cbar.set_label("SNR (unitless)"); #tabel the colorbar
...
#    #cbar.set_clim( (-ISR_plotLimValu,ISR_plotLimValu) ); #create useful tick marks
    ...
    string_Title = 'MISA SNR with High-pass Filter '+str(filter_cutoffPeriod)+' hr Cutoff - Hours '+str(np.min(time_cutout_range))+' to '+str(np.max(time_cutout_range)); #create mecha title
    ...
    fig.subplots_adjust(left = 0.060, right = 0.98, top = 0.96, bottom = 0.065 , hspace = 0.225); #sets padding to small numbers for minimal white space
