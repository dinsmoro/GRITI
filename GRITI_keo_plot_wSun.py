...
def GRITI_keo_plot_wSun(keoData,timeUnique,plotLimValu,colorMap,plotLatRange,plotLongRange,latMillstone,longMillstone,
    ...
    string_Title = avg_anyAngle_dataType+' Averaged on Angle of '+str(np.round(avg_anyAngle,2))+' deg and Width of '+ \
        str(np.round(avg_anyAngle_Width,2))+' arcdeg (Sun Location is Orange Line)'; #create mecha title
    ...
    for i in range(0,timeUnique.size):
        ...
        #END IF
    #END FOR i
    # ax.plot( (timeUnique - dateRange_dayNum_zeroHr[1])*24, sunPlot,c='xkcd:orange',linewidth=2.5); #plots a point with an orange line
    indexes2plot = np.hstack( (np.arange(0,timeUnique.size,10),(timeUnique.size-1,)) ); #plot select stars
    ax.plot( ((timeUnique - dateRange_dayNum_zeroHr[1])*24)[indexes2plot], sunPlot[indexes2plot], c='xkcd:orange', marker='*', markersize=20, linestyle="None"); #plots a point with an orange line
