...
def GRITI_TEC_avgPt_HP_timeMatch_POPL_scargleWithISR_cutOut(avgPt_vTEC_timeMatch_HP_cutOut,avgPt_vTEC_timeMatch_time_cutOut,Zenith_time_cutOut,Zenith_POPL_hp_altAvgd_cutOut,MISA_time_cutOut,MISA_POPL_hp_altAvgd_cutOut,pointAltitude,avgPt_ISRavgAlt,time_cutout_range,filter_cutoffPeriod,avgPt_pointRadius,plot_Period_Lim,dateRange,dateRange_dayNum,dateRange_dayNum_zeroHr,FONT_titleFM,FONT_axisTick,FONT_axisLabelFM,PLOT_lineWidth):
    ...
    Zenith_scargPeriod, Zenith_scargPower, Zenith_scarggf = subfun_lombscargle((Zenith_time_cutOut - dateRange_dayNum_zeroHr[1])*24 , Zenith_POPL_hp_altAvgd_cutOut); #scargle that data
    Zenith_scargPeriod = Zenith_scargPeriod*60; #min, adjust the period out from hrs to minutes (since hrs goes in)
    
    MISA_scargPeriod, MISA_scargPower, MISA_scarggf = subfun_lombscargle((MISA_time_cutOut - dateRange_dayNum_zeroHr[1])*24 , MISA_POPL_hp_altAvgd_cutOut); #scargle that data
...
#    ax[0].set_xlabel("Periods (min)",fontproperties=FONT_axisLabelFM); #set the x axis label
    ...
    #~~~~~~~~~~ ZENITH STUFF ~~~~~~~~~~
    #-----ZENITH lomb-scargle of POPL high-passed and averaged +/-25 km at 300km-----
...
#    ax[1].set_xlabel("Periods (min)",fontproperties=FONT_axisLabelFM); #set the x axis label
    ...
    string_Title = 'Scargle of Zenith POPL HP''d w/ '+str(filter_cutoffPeriod)+' hr cutoff & Avg''d +/- '+str(avgPt_ISRavgAlt)+' around '+str(pointAltitude)+' km altitude - Time Cutout '+str(np.min(time_cutout_range))+' to '+str(np.max(time_cutout_range))+' hrs'; #create mecha title
    ...
    #~~~~~~~~~~ MISA STUFF ~~~~~~~~~~
    #-----MISA lomb-scargle of POPL high-passed and averaged +/- 25km at 300km-----
