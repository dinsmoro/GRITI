...
def sunAlsoRises(dateRange_full,lat,long):
    ...
    tzInt = np.empty(dateRange_full.shape[0],dtype=np.int16); #preallocate
    for j in range(0,dateRange_full.shape[0]):
        ...
        tzInt[j] = np.int16(tzString[j]); #record as a number
    #END FOR j
    ...
    tzString = []; #prep a list
    tzInt = np.empty(dateRange_fullPad.shape[0],dtype=np.int16); #preallocate
