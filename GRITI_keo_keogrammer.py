...
from GRITI_keo_plot_area import GRITI_keo_plot_area
from GRITI_keo_plot_dataDensity import GRITI_keo_plot_dataDensity
...
def GRITI_keo_keogrammer(data_data ,data_time, data_lat, data_long, data_timeUnique, data_timeRef, dates,
        ...
        FLG_fancyPlot=0,FLG_disablePlot=0,FLG_dataDensity=0,FLG_disableText=0,FLG_disableCache=0,FLG_useRightExact=0):
    ...
    if( (FLG_useRightExact == 1) & ((keo_angle == 0) | (keo_angle == 270) | (keo_angle == 90) | (keo_angle == 180)) ): #don't use this on the reg, ray trace is x3 faster
        ...
        if( (keo_angle == 0) | (keo_angle == 180) ):
            ...
            keo = GRITI_keo_subfun_simpleTrace_long(time_limd,data_timeUnique,pplat_limd,pplong_limd,splits,latLims,vTEC_limd,keo_N); #do the below alg faster
            # #time to crunch keo
            ...
            # #END IF
            if( np.all(keepr) == 0 ):
                keo = GRITI_keo_subfun_simpleTrace_lat(data_time[keepr],data_timeUnique,data_lat[keepr],data_long[keepr],splits,longLims,data_data[keepr],keo_N); #do the below alg faster
            else:
                keo = GRITI_keo_subfun_simpleTrace_lat(data_time,data_timeUnique,data_lat,data_long,splits,longLims,data_data,keo_N); #do the below alg faster
            #END IF
    ...
    #    #END FOR j
        ...
        if( FLG_disableCache == 0 ):
            import pickle
            cachedName = keo_name+str(dates['date range dayNum']).replace('\n','')+str(plotLatRange)+str(plotLongRange)+str(np.round(keo_angle,2)).rstrip('0').rstrip('.')+'_'+str(np.round(keo_width,2)).rstrip('0').rstrip('.')+'_'+str(keo_N)+'_'+str(keo_45vsLatLong)+'_'+str(np.sum(keepr))+str(settings_dataSpecific['source to use']); #make a big string to uniquely identify this keogram
        ...
        if( FLG_cachedAvail == 0 ):
            keo = GRITI_keo_subfun_raytrace(time_limd,data_timeUnique,pplat_limd,pplong_limd,temp_Lat_List,temp_Long_List,vTEC_limd,keo_N,sqrtApproxAo,sqrtApproxBo,P1P2Vxy,P1P4Vxy,P3P2Vxy,P3P4Vxy,P1P2Vm,P1P4Vm,P3P2Vm,P3P4Vm,P1P2P1P4dotCosT,P3P2P3P4dotCosT); #call a function to avg all the TEC together into bands
            #save the data
            if( FLG_disableCache == 0 ):
