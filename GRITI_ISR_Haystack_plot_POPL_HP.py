...
def GRITI_ISR_Haystack_plot_POPL_HP(Zenith_time,Zenith_height,Zenith_POPL_hp,MISA_time,MISA_height,MISA_POPL_hp,filter_cutoffPeriod,ISR_RTI_heightLimValues,ISR_POPL_plotLimValu,dateRange,dateRange_dayNum,dateRange_dayNum_zeroHr,FONT_titleFM,FONT_axisTick,FONT_axisLabelFM):
    ...
    pltHelprX, pltHelprY = np.meshgrid( (Zenith_time - dateRange_dayNum_zeroHr[1])*24, \
                Zenith_height);
    im = ax[0].pcolormesh(pltHelprX , pltHelprY , Zenith_POPL_hp , vmin=np.min(ISR_POPL_plotLimValu) , vmax=np.max(ISR_POPL_plotLimValu) , cmap='gray' , shading='gouraud'); # pseudocolor plot "stretched" to the grid
    ...
    pltHelprX, pltHelprY = np.meshgrid( (MISA_time - dateRange_dayNum_zeroHr[1])*24, \
                MISA_height);
    im = ax[1].pcolormesh(pltHelprX , pltHelprY , MISA_POPL_hp , vmin=np.min(ISR_POPL_plotLimValu) , vmax=np.max(ISR_POPL_plotLimValu) , cmap='gray', shading='gouraud'); # pseudocolor plot "stretched" to the grid
    cbar = fig.colorbar(im, cax=cax, orientation='vertical'); #create a colorbar using the prev. defined cax
    cbar.mappable.set_clim(vmin=np.min(ISR_POPL_plotLimValu), vmax=np.max(ISR_POPL_plotLimValu));
    #cbar.ax.set_yticklabels(np.round(np.linspace(-ISR_POPL_plotLimValu,ISR_POPL_plotLimValu,5), len(str(ISR_POPL_plotLimValu).split('.')[1])+1 )); #create useful tick marks
    cbar.set_label("POPL [$e^-/m^3$]"); #tabel the colorbar
